...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{LMM validation (K>=3)}
...
rows <- row_result(
  ...
  "fixed_effect_max_abs_delta", observed, 0.01, "practical",
  paste("Cluster membership is used internally; per-cluster residuals/BLUPs",
        "are not returned. The default K>=3 route uses the OT-efficient",
        "moment/cluster-mean GLS approximation; set",
        "options(dsvert.lmm_k3.profile_mode='profile') for the exhaustive",
        "REML-matching profile."),
  result$runtime_s)
