test_that("binomial GH5 grids bind variance-major candidates and full patient sensitivity", {
  for (owners in c(2L, 3L, 5L)) {
    f <- .grouped_cross_client_fixture("binomial_glmm", owners = owners)
    raw <- f$raw
...
test_that("binomial GH5 variance grids reject ambiguous or uncertified values", {
  f <- .grouped_cross_client_fixture("binomial_glmm")
  ...
  numeric <- .dsvert_dp_grouped_grid_cross_numeric("binomial_glmm",
...
test_that("binomial GH5 postprocessing selects variance and keeps first ties", {
  f <- .grouped_cross_client_fixture("binomial_glmm")
  raw <- f$raw
  raw$parameters <- list(variance_grid = list(0, .25), quadrature = "gh5_fixed_v1")
  spec <- .dsvert_dp_grouped_grid_cross_spec(raw, f$policy, f$schema)
...
test_that("binomial GH5 public artifacts rebuild numeric caps and lattice identity", {
  f <- .grouped_cross_client_fixture("binomial_glmm")
  raw <- f$raw
  raw$parameters <- list(variance_grid = list(0, .25), quadrature = "gh5_fixed_v1")
  spec <- .dsvert_dp_grouped_grid_cross_spec(raw, f$policy, f$schema)
...
test_that("new binomial variance-grid contracts reject p4 and J5 before execution", {
  f <- .grouped_cross_client_fixture("binomial_glmm", owners = 5L)
  raw <- f$raw
