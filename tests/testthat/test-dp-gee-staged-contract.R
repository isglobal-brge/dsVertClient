test_that("fixed-rho staged GEE binds native cap scales and keeps legacy contracts", {
  for (family in c("binomial_gee", "poisson_gee")) {
    ...
    legacy <- .dsvert_dp_grouped_grid_cross_spec(f$raw, f$policy, f$schema)
    ...
    unsigned <- list(version = f$contract$version, spec = spec,
      artifact = .dsvert_dp_grouped_grid_cross_artifact(spec))
    unsigned$source_contract <- .dsvert_dp_grouped_grid_cross_source_contract(spec, unsigned$artifact)
...
test_that("GEE n2000 descriptor is an explicit bounded measurement domain", {
  f <- .grouped_cross_client_fixture("poisson_gee")
  ...
  for (mutation in list(
    ...
    expect_error(.dsvert_dp_grouped_grid_cross_spec(mutation(raw), policy, f$schema),
      class = "dsvert_dp_public_failure")
  }
})
...
test_that("fixed-rho sensitivity counts affected clusters, not cluster capacity", {
  for (family in c("binomial_gee", "poisson_gee")) {
    f <- .grouped_cross_client_fixture(family, "exchangeable")
    raw <- f$raw; raw$parameters$composition <- "staged_fixed_rho_v1"
    small <- .dsvert_dp_grouped_grid_cross_spec(raw, f$policy, f$schema)
    raw$grouping$cluster_capacity <- 500
    policy <- f$policy; policy$unit_capacity <- 2000
    large <- .dsvert_dp_grouped_grid_cross_spec(raw, policy, f$schema)
    ...
    replacement <- .dsvert_dp_grouped_grid_cross_spec(raw, policy, f$schema)
  ...
  for (family in c("binomial_gee", "poisson_gee")) {
    for (correlation in c("independence", "exchangeable", "ar1")) {
      ...
      for (rho in values) {
        ...
        expect_equal(.dsvert_dp_grouped_grid_cross_parameters(parameters, family), parameters)
        for (mutation in list(
            function(x) { x$rho <- c(0, .25); x },
            function(x) { x$rho <- .125; x },
            function(x) { x$rho <- "estimated"; x },
            function(x) { x$estimator <- "private_moment"; x })) {
          expect_error(.dsvert_dp_grouped_grid_cross_parameters(mutation(parameters), family),
            class = "dsvert_dp_public_failure")
        }
      }
    }
    expect_error(.dsvert_dp_grouped_grid_cross_parameters(list(correlation = "independence",
