...
test_that("LMM help keeps both historical names on the signed moment route", {
  ...
  expect_match(k3, "no K>=3 REML/profile fit", fixed = TRUE)
})

test_that("the K>=3 compatibility printer does not imply a profile fit", {
  fit <- list(
  ...
  expect_match(paste(printed, collapse = "\n"), "no K>=3 REML/profile fit",
               fixed = TRUE)
  expect_false(any(grepl("REML 1-D profile", printed, fixed = TRUE)))
})
