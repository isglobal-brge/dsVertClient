...
test_that("real Synopsis Count is plausible and Rock-replayable at K=2/3/5", {
  ...
  for (k in .synopsis_real_e2e_peer_counts()) {
    ...
    expect_false(isTRUE(first$one_joint_draw))
    expect_identical(first$implementation, .DSVERT_CLIENT_VECTOR_BACKEND)
    expect_null(first$backend_selection)
    expect_gt(first$synopsis_coordinate_count, 1L)
    ...
    route_count <- function(data_name, server = NULL, datasources = NULL,
                            .aggregate) {
      count(data_name, server, datasources, dispatch)
    }
    public <- testthat::with_mocked_bindings(
      .dsvert_dp_count_impl = route_count,
      ds.vertDPCount("data_peer_a", "peer_b", conns),
      .package = "dsVertClient")
    expect_s3_class(public, "ds.vertDPCount")
    expect_identical(public$value, first$value)
