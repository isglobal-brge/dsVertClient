test_that("staged grouped receipts bind both peers to family plan terminal and candidate shape", {
  for (kind in c("lmm", "glmm")) {
  artifact <- list(analysis_id = "lmm", family = if (kind == "lmm") "lmm" else "binomial_glmm",
    coordinate_count = 10L,
  ...
  receipt <- function(peer, phase, extra = list()) c(list(
    version = paste0("dsvert-", kind, "-staged-receipt-v1"), phase = phase, capsule_id = strrep("c", 64),
  ...
  extra <- list(operation_id = paste0("op_", strrep("a", 32)), source_key = paste0("exact_gc_in_", strrep("a", 32)),
    output_key = paste0("exact_gc_out_", strrep("a", 32)), operation = paste0("grouped-", kind, "-staged-v1"),
    purpose = paste0("grouped-", kind, "-staged-v1/", strrep("1", 64)), vector_len = 10L, persisted = TRUE)
  ...
  swapped <- values
  swapped[[1L]]$version <- if (kind == "glmm") "dsvert-lmm-staged-receipt-v1" else "dsvert-glmm-staged-receipt-v1"
  expect_error(.dsvert_dp_lmm_cross_receipts(wire(swapped), context, artifact, "complete"))
