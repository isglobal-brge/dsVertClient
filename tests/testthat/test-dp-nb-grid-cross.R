...
test_that("NB2 signed contracts retain the frozen source ABI and both authorities", {
  ...
  decoded <- jsonlite::fromJSON(.dsvert_joint_dp_client_json(got), simplifyVector = FALSE)
...
test_that("NB2 candidate identities bind theta and reject encoded collisions", {
  f <- .nb_grid_cross_fixture("dsVertClient", beta_grid = list(c(0, 1, 0), c(0, 1, 0)),
                             theta_grid = c(1, 2))
...
test_that("NB2 client entry validates cross-owner names then fails before DSI", {
  ...
  expect_error(dp_nb_grid(peer_a$y ~ peer_a$x + peer_b$z, "cohort", "nb_grid",
    bad, f$policy, f$schema), class = "dsvert_dp_public_failure")
  expect_false(f$registration$production_release_registered)
})

test_that("NB2 postprocessing selects only signed public beta/theta candidates", {
  f <- .nb_grid_cross_fixture("dsVertClient")
  spec <- f$contract$spec
  got <- f$registration$postprocess(c(100, 20), spec)
  expect_identical(got$selected_candidate, 2L)
  expect_equal(got$theta, spec$theta_grid[[2L]])
  expect_equal(unname(got$normalized_coefficients), unlist(spec$beta_grid[[2L]]))
  expect_equal(got$coefficients[["peer_a$x"]], 1 / 6)
  expect_identical(got$inference, "unavailable_for_finite_grid_nb2")
  expect_null(got$standard_errors)
  expect_identical(f$registration$postprocess(c(20, 20), spec)$selected_candidate, 1L)
  expect_error(f$registration$postprocess(c(NA, 1), spec), class = "dsvert_dp_public_failure")
  expect_error(f$registration$postprocess(c(0.5, 1), spec), class = "dsvert_dp_public_failure")
})
...
test_that("NB2 pure-R limb oracle exactly matches certified Go fixtures", {
  value <- jsonlite::fromJSON(system.file("cross-grid-nb", "numeric_profile_nb_v1.json",
                                         package = "dsVertClient"), simplifyVector = FALSE)
