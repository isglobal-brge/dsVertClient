...
|     | server |   n | columns                |
|:----|:-------|----:|:-----------------------|
| s1  | s1     |  60 | patient_id, w1         |
| s2  | s2     |  60 | patient_id, w2         |
| s3  | s3     |  60 | patient_id, tr, ipw, y |
...
#>   y_var 'tr' found on: s3
#>   s1: w1
#>   s2: w2
#>   s3 (label): (response only)
#> === Encrypted-Label BCD-IRLS for binomial GLM ===
#> Observations: 60, Variables: 2, Partitions: 3
#> Label server: s3 (holds 'tr')
...
#> 
#> [Phase 3] K>=3 ring63 DCF + Beaver L-BFGS (family=binomial, n=60, intervals=150, lambda=1.0e-04)
#>   [Input Sharing] Sharing features+y with DCF parties...
#>   [Input Sharing] Complete (p_total=2, 0.3s)
#>   [DCF] Server s2 generating keys (n=60, 150 intervals)...
#>   [DCF] Keys distributed (47.3s)
#>     [1.1] Computing eta shares...
#>     [1.2] DCF wide spline...
#>     [1.3] Beaver gradient...
#>   Iter 1: ||grad||=0.1681, step=0.30, diff=3.48e-02, theta=[-0.030, 0.035] (3.4s)
...
#>     [3.2] DCF wide spline...
#>     [3.3] Beaver gradient...
#>   Iter 3: ||grad||=0.0125, step=1.00, diff=4.01e-02, theta=[-0.430, 0.473] (3.1s)
#>   Iter 4: ||grad||=0.0019, step=1.00, diff=7.63e-03, theta=[-0.435, 0.481] (3.2s)
#>   Iter 5: ||grad||=0.0000, step=1.00, diff=1.94e-04, theta=[-0.435, 0.481] (3.2s)
#>   Iter 6: ||grad||=0.0000, step=1.00, diff=4.40e-05, theta=[-0.435, 0.481] (3.2s)
#>   Converged after 6 iterations (diff = 4.40e-05)
#>   [Ring63-DCF-Beaver] Converged after 6 iters (total 19.3s)
#> 
#> [Phase 5] Secure deviance (Beaver): NA
#> 
...
#> Registering weights 'ipw' on server s3
#> 
...
#>     [2.1] Computing eta shares...
#>     [2.3] Beaver gradient...
#>   Iter 2: ||grad||=0.6670, step=1.00, diff=2.92e-01, theta=[-0.125, 0.772] (0.5s)
#>     [3.1] Computing eta shares...
#>     [3.3] Beaver gradient...
#>   Iter 3: ||grad||=0.0271, step=1.00, diff=1.20e-02, theta=[-0.121, 0.774] (0.4s)
#>   Iter 4: ||grad||=0.0033, step=1.00, diff=1.24e-03, theta=[-0.120, 0.773] (0.6s)
#>   Iter 5: ||grad||=0.0004, step=1.00, diff=1.93e-04, theta=[-0.120, 0.773] (0.5s)
#>   Iter 6: ||grad||=0.0001, step=1.00, diff=7.91e-05, theta=[-0.120, 0.773] (0.5s)
#>   Converged after 6 iterations (diff = 7.91e-05)
#>   [Ring63-DCF-Beaver] Converged after 6 iters (total 2.7s)
#> 
#> [Phase 5] Secure deviance (Beaver): NA
#> 
...
|:---|:---|:---|:---|:---|---:|---:|:---|:---|---:|
| K\>=3 | ds.vert.ipw(precision=‘high’) | synthetic confounded IPW fixture | central weighted lm using same weights | weighted_outcome_coef_abs_delta | 0.0001785 | 0.001 | strict-practical | PASS | 73.3 |
