...
#>   Iter 9: ||grad||=0.0006, step=1.00, diff=3.67e-04, theta=[-0.000, 0.341] (0.2s)
#>   Iter 10: ||grad||=0.0004, step=1.00, diff=8.33e-04, theta=[0.000, 0.342] (0.2s)
...
#>   Iter 13: ||grad||=0.0002, step=1.00, diff=6.46e-05, theta=[0.000, 0.342] (0.2s)
#>   Converged after 13 iterations (diff = 6.46e-05)
...
#>   npreg               0.0108     0.0199      0.543   0.586997
#>   glu                 0.0047     0.0015      3.075   0.002105 **
#>   bp                  0.0014     0.0045      0.317   0.751537
#>   skin                0.0070     0.0061      1.136   0.255938
#> [Auto-detect] Querying server columns...
#>   y_var 'diabetes' found on: s2
#>   s1: age, bmi, ped
#>   s2 (label): glu, bp, skin
#> === Encrypted-Label BCD-IRLS for gaussian GLM ===
#> Observations: 60, Variables: 6, Partitions: 2
#> Label server: s2 (holds 'diabetes')
#> Non-label servers: s1
#> 
#> [Phase 0] Transport key setup (2 servers)...
...
#>   Gaussian one-shot: X^T X + X^T y via Beaver, direct solve
#>   [Input Sharing] Generating additive secret shares (2 servers)...
#>   [Input Sharing] Complete: p_coord=3, p_nl=3 (0.1s)
#> 
#> [Phase 3] K=2 L-BFGS iterations (p=6, n=60, lambda=1.0e-04)
#>   [L-BFGS] sum_res=-0.0002, ||grad||=0.869018, grad_range=[-0.5051, -0.0000]
#>   Iter 1: ||grad||=0.8690, step=0.30, diff=1.52e-01, theta=[0.000, 0.152] (0.2s)
#>   [L-BFGS] sum_res=-0.0005, ||grad||=0.427460, grad_range=[-0.2639, -0.0000]
#>   Iter 2: ||grad||=0.4275, step=1.00, diff=1.76e-01, theta=[0.000, 0.328] (0.2s)
#>   [L-BFGS] sum_res=-0.0000, ||grad||=0.082505, grad_range=[-0.0562, 0.0409]
#>   Iter 3: ||grad||=0.0825, step=1.00, diff=4.47e-02, theta=[0.000, 0.346] (0.3s)
...
#>   Iter 10: ||grad||=0.0002, step=1.00, diff=4.09e-04, theta=[0.000, 0.343] (0.2s)
#>   Iter 11: ||grad||=0.0003, step=1.00, diff=2.21e-04, theta=[0.000, 0.343] (0.3s)
#>   Iter 12: ||grad||=0.0001, step=1.00, diff=8.23e-05, theta=[0.000, 0.343] (0.2s)
#>   Converged after 12 iterations (diff = 8.23e-05)
#>   [SE] Computing Hessian (central differences)...
#>     [SE] Column 1/7
#>     [SE] Column 2/7
#>     [SE] Column 3/7
...
#>   [Deviance] = 30.7039
#> 
#> [Phase 5] Secure deviance (K=2 Beaver): 30.7039
#> 
#> Deviance: 6.3403, Null deviance: 12.1833, Pseudo R2: 0.4796
#> 
#> Coefficients:
#>                     Estimate  Std.Error    z value   Pr(>|z|)
#>   (Intercept)        -1.2918     0.3467     -3.725   0.000195 ***
#>   age                 0.0133     0.0046      2.878   0.004006 **
#>   bmi                 0.0029     0.0096      0.299   0.764809
#>   ped                 0.3148     0.1173      2.684   0.007276 **
#>   glu                 0.0047     0.0015      3.106   0.001895 **
#>   bp                  0.0024     0.0041      0.578   0.563408
#>   skin                0.0067     0.0061      1.110   0.266903
...
|:---|:---|:---|:---|:---|---:|---:|:---|:---|---:|
| K=2 | ds.vert.confint / ds.vert.wald / ds.vert.contrast / ds.vert.lr | MASS::Pima.tr fixture | manual algebra on ds.vert.glm output | algebra_max_abs_delta | 0 | 0 | strict-precise | PASS | 16.1 |
