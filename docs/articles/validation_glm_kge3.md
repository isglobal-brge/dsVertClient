...
The public frontdoor routes to the protected GLM engine. This vignette
uses the Gaussian fixture from the matrix, where the centralized target
is exact least squares.

## Mathematical target

For the Gaussian identity-link case, beta solves X’X beta = X’y. The
distributed route reconstructs only aggregate score/Hessian information
needed for beta.

## Disclosure review

The analyst receives coefficients and model-level
covariance/diagnostics. Eta, residuals, row scores, and fitted values
are not returned.
...
#>   Iter 5: ||grad||=0.0188, step=1.00, diff=1.88e-02, theta=[0.000, 0.345] (0.4s)
...
#>   Iter 13: ||grad||=0.0002, step=1.00, diff=6.55e-05, theta=[0.000, 0.342] (0.3s)
#>   Converged after 13 iterations (diff = 6.55e-05)
#>   [Ring63-DCF-Beaver] Converged after 13 iters (total 3.4s)
...
#> 
#> Deviance: 6.3033, Null deviance: 12.1833, Pseudo R2: 0.4826
#> 
#> Coefficients:
#>                     Estimate  Std.Error    z value   Pr(>|z|)
#>   (Intercept)        -1.2307     0.3666     -3.357   0.000788 ***
#>   age                 0.0118     0.0054      2.180   0.029221 *
#>   bmi                 0.0034     0.0098      0.350   0.726392
#>   ped                 0.3141     0.1181      2.660   0.007822 **
#>   skin                0.0070     0.0061      1.136   0.255901
#>   npreg               0.0108     0.0199      0.543   0.586961
#>   glu                 0.0047     0.0015      3.075   0.002103 **
#>   bp                  0.0014     0.0045      0.317   0.751516
...
|:---|:---|:---|:---|:---|---:|---:|:---|:---|---:|
| K\>=3 | ds.vert.glm | MASS::Pima.tr fixture | stats::lm | coef_max_abs_delta | 5.93e-05 | 0.001 | strict-practical | PASS | 10 |
