...
| s3  | s3     |  60 | patient_id, x3, y_cls, high_ind, low_ind, med_ind |
...
#>   y_var 'low_ind' found on: s3
#>   s1: x1
#>   s2: x2
#>   s3 (label): x3
#> === Encrypted-Label BCD-IRLS for binomial GLM ===
#> Observations: 60, Variables: 3, Partitions: 3
#> Label server: s3 (holds 'low_ind')
...
#> 
#> [Phase 3] Ring63 Beaver Gradient (K=3 servers, family=binomial)...
#> 
#> [Phase 3] K>=3 ring63 DCF + Beaver L-BFGS (family=binomial, n=60, intervals=100, lambda=1.0e-04)
#>   [Input Sharing] Sharing features+y with DCF parties...
#>   [Input Sharing] Complete (p_total=3, 0.2s)
#>   [DCF] Server s2 generating keys (n=60, 100 intervals)...
#>   [DCF] Keys distributed (28.9s)
...
#>     [2.2] DCF wide spline...
#>     [2.3] Beaver gradient...
#>   Iter 2: ||grad||=0.1592, step=1.00, diff=6.18e-01, theta=[-0.668, 0.171] (2.3s)
#>     [3.1] Computing eta shares...
#>     [3.2] DCF wide spline...
#>     [3.3] Beaver gradient...
#>   Iter 3: ||grad||=0.0079, step=1.00, diff=2.82e-02, theta=[-0.696, 0.189] (2.3s)
#>   Iter 4: ||grad||=0.0009, step=1.00, diff=3.16e-03, theta=[-0.699, 0.191] (2.5s)
#>   Iter 5: ||grad||=0.0000, step=1.00, diff=1.10e-04, theta=[-0.699, 0.191] (2.3s)
#>   Iter 6: ||grad||=0.0000, step=1.00, diff=2.81e-05, theta=[-0.699, 0.191] (2.2s)
#>   Converged after 6 iterations (diff = 2.81e-05)
#>   [Ring63-DCF-Beaver] Converged after 6 iters (total 14.0s)
...
#>   [Deviance] = 75.9647
#> 
#> [Phase 5] Secure deviance (Beaver): 75.9647
#> 
#> Deviance: 75.9647, Null deviance: NA, Pseudo R2: NA
#> 
#> Coefficients:
#>                     Estimate  Std.Error    z value   Pr(>|z|)
#>   (Intercept)        -0.6763     0.2815     -2.403   0.016277 *
#>   x1                  0.1607     0.2318      0.693   0.488083
#>   x2                  0.0380     0.3001      0.126   0.899370
#>   x3                  0.0127     0.2051      0.062   0.950566
#> [ds.vertMultinom] fitting class 'med' vs rest (indicator 'med_ind')
#> [Auto-detect] Querying server columns...
#>   y_var 'med_ind' found on: s3
#>   s1: x1
#>   s2: x2
#>   s3 (label): x3
#> === Encrypted-Label BCD-IRLS for binomial GLM ===
#> Observations: 60, Variables: 3, Partitions: 3
#> Label server: s3 (holds 'med_ind')
#> Non-label servers: s1, s2
#> 
#> [Phase 0] Transport key setup (3 servers)...
#>   [Key Setup] Transport keys exchanged (3 servers, 0.2s)
#> 
#> [Phase 1] Standardizing features across 3 servers...
#>   [Standardize] 3 total features standardized (y raw, 0.0s)
#> 
...
#>   [Input Sharing] Complete (p_total=3, 0.2s)
#>   [DCF] Server s2 generating keys (n=60, 100 intervals)...
#>   [DCF] Keys distributed (29.2s)
#>     [1.1] Computing eta shares...
#>     [1.2] DCF wide spline...
#>     [1.3] Beaver gradient...
#>   Iter 1: ||grad||=0.1722, step=0.30, diff=5.00e-02, theta=[-0.050, 0.000] (2.4s)
#>     [2.1] Computing eta shares...
#>     [2.2] DCF wide spline...
#>     [2.3] Beaver gradient...
#>   Iter 2: ||grad||=0.1594, step=1.00, diff=6.19e-01, theta=[-0.669, 0.025] (2.3s)
#>     [3.1] Computing eta shares...
#>     [3.2] DCF wide spline...
#>     [3.3] Beaver gradient...
#>   Iter 3: ||grad||=0.0084, step=1.00, diff=2.81e-02, theta=[-0.697, 0.028] (2.3s)
#>   Iter 4: ||grad||=0.0012, step=1.00, diff=3.83e-03, theta=[-0.700, 0.032] (2.3s)
#>   Iter 5: ||grad||=0.0001, step=1.00, diff=5.37e-04, theta=[-0.700, 0.032] (2.5s)
#>   Iter 6: ||grad||=0.0001, step=1.00, diff=1.30e-04, theta=[-0.700, 0.032] (2.2s)
#>   Iter 7: ||grad||=0.0000, step=1.00, diff=4.94e-05, theta=[-0.700, 0.032] (2.3s)
#>   Converged after 7 iterations (diff = 4.94e-05)
#>   [Ring63-DCF-Beaver] Converged after 7 iters (total 16.3s)
#>   [SE] Computing exact Hessian (central differences)...
#>     [SE] Column 1/4
#>     [SE] Column 2/4
#>     [SE] Column 3/4
...
#>   [Deviance] = 75.9352
#> 
#> [Phase 5] Secure deviance (Beaver): 75.9352
#> 
#> Deviance: 75.9352, Null deviance: NA, Pseudo R2: NA
#> 
#> Coefficients:
#>                     Estimate  Std.Error    z value   Pr(>|z|)
#>   (Intercept)        -0.6859     0.2818     -2.434   0.014935 *
#>   x1                  0.0099     0.2332      0.043   0.966070
#>   x2                 -0.2128     0.2937     -0.724   0.468776
#>   x3                  0.0230     0.2054      0.112   0.910845
#> [MultinomJointNewton] session multinomJoint_1778057611_238796008  n=60  classes=3  DCF=(s3,s1)  p=4
#> [MnlJoint] iter 1 |g|_L2=3.283e-02 |g|_max=2.569e-02 |step|_pre=8.053e-02 |step|_post=8.053e-02 beta_max=2.393e-01 dt=24.7s
#> [MnlJoint] iter 2 |g|_L2=1.164e-02 |g|_max=9.235e-03 |step|_pre=2.847e-02 |step|_post=2.847e-02 beta_max=2.496e-01 dt=24.8s
#> [MnlJoint] iter 3 |g|_L2=4.255e-03 |g|_max=3.334e-03 |step|_pre=1.019e-02 |step|_post=1.019e-02 beta_max=2.534e-01 dt=24.5s
#> [MnlJoint] iter 4 |g|_L2=1.632e-03 |g|_max=1.242e-03 |step|_pre=3.938e-03 |step|_post=3.938e-03 beta_max=2.549e-01 dt=24.8s
#> [MnlJoint] iter 5 |g|_L2=6.536e-04 |g|_max=4.917e-04 |step|_pre=1.602e-03 |step|_post=1.602e-03 beta_max=2.555e-01 dt=25.1s
#> [MnlJoint] iter 6 |g|_L2=2.582e-04 |g|_max=1.989e-04 |step|_pre=6.571e-04 |step|_post=6.571e-04 beta_max=2.558e-01 dt=24.6s
#> [MnlJoint] iter 7 |g|_L2=1.074e-04 |g|_max=6.484e-05 |step|_pre=3.029e-04 |step|_post=3.029e-04 beta_max=2.558e-01 dt=25.4s
#> [MnlJoint] iter 8 |g|_L2=3.870e-05 |g|_max=2.609e-05 |step|_pre=1.556e-04 |step|_post=1.556e-04 beta_max=2.558e-01 dt=24.3s
#> [MnlJoint] iter 9 |g|_L2=4.165e-05 |g|_max=3.476e-05 |step|_pre=8.014e-05 |step|_post=8.014e-05 beta_max=2.559e-01 dt=24.7s
#> [MnlJoint] iter 10 |g|_L2=4.697e-05 |g|_max=3.778e-05 |step|_pre=4.981e-05 |step|_post=4.981e-05 beta_max=2.559e-01 dt=24.5s
...
|:---|:---|:---|:---|:---|---:|---:|:---|:---|---:|
| K\>=3 | ds.vert.multinom | balanced soft-signal synthetic 3-class fixture | nnet::multinom probabilities | class_probability_max_abs_delta | 3.28e-05 | 0.001 | strict-practical | PASS | 427.3 |
