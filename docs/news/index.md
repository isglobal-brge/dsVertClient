...
- **`ds.vertCox.k3`** — Cox K=3 via the Allison-1982 Poisson trick with
  offset + 100 baseline dummies; L-BFGS step-damping + backtracking
  guard (CGD).
...
- **`ds.vertGLMM`** — Laplace-approximated binomial GLMM (single random
  intercept; stretch deliverable, moment-matching variance recovery).
