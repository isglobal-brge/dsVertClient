# Query Server Identity Public Keys

Queries all connected DataSHIELD servers for their Ed25519 identity
public keys. Used by administrators to discover PKs for configuring the
name-bound `dsvert.trusted_peers` option on each server. A new or
changed key must be verified independently at the corresponding server
console before updating any pin. The analyst/relay is not a trusted
source for accepting a replacement identity.

## Usage

``` r
ds.getIdentityPks(datasources = NULL)
```

## Arguments

- datasources:

  DataSHIELD connections. If NULL, uses all available.

## Value

Named list: server_name -\> identity_pk (base64url string).

## Examples

``` r
if (FALSE) { # \dontrun{
pks <- ds.getIdentityPks(datasources = conns)
# After out-of-band verification, configure every server with a named map
# of the *other* logical peers. For example, on site_a:
site_a_pins <- unlist(pks[setdiff(names(pks), "site_a")])
names(site_a_pins) <- setdiff(names(pks), "site_a")
# Persist site_a_pins as the server option dsvert.trusted_peers, restart
# the affected DataSHIELD service, and repeat for every participating site.
} # }
```
