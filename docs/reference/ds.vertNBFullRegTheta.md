...
- variant:
  ...
  Character. `"iid_mu"` returns the unmodified `ds.vertNB` result.
  `"corrected"` (default) applies the aggregate variance correction
...
`$variance_correction` (the \\\hat V\_\mu\\ used).
