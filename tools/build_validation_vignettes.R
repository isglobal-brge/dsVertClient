...
method_pages <- list(
  psi = list(
    ...
    functions = "ds.vert.align(), ds.vert.is_aligned()",
  ...
  contingency = list(
    ...
    functions = "ds.vert.chisq(), ds.vert.fisher(), ds.vert.chisq_cross()",
  ...
  inference = list(
    ...
    method = "Inference helpers post-process the beta, covariance, deviance, and degrees of freedom already returned by ds.vert.glm.",
    ...
    reference = "Manual algebra on ds.vert.glm output.",
  ...
  lasso = list(
    ...
    reference = "The prime ds.vert.glm coefficients at lambda = 0.",
  ...
  glmm = list(
    ...
    method = "The product route is aggregate PQL for a binomial random-intercept GLMM. The executable vignette runs the public default PQL route and checks fixed-effect accuracy plus convergence diagnostics.",
    math = "A PQL step alternates protected binomial working-response updates with aggregate weighted mixed-model normal equations. The validation compares fixed effects to a centralized PQL fit and asserts that the distributed PQL quality flag is ok.",
    fixture = "Synthetic mixed binomial random-intercept fixture with cluster sizes above privacy thresholds.",
    reference = "MASS::glmmPQL on the pooled fixture.",
...
write_page <- function(id, meta) {
  ...
  lines <- c(
    ...
    "",
    "```{r evidence}",
    paste0("rows <- run_validation(\"", id, "\", force = force_run, trace = TRUE)"),
