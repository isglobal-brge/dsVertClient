...
suffix <- Sys.getenv("DSVERT_REGISTRY_SUFFIX", "")
library(dsVertClient)
status <- ds.vertMethodStatus()
write.csv(status, file.path(out, paste0("method_status_v130", suffix, ".csv")), row.names = FALSE)
print(names(status))
...
jsonlite::write_json(summary, file.path(out, paste0("registry_counts_v130", suffix, ".json")), pretty = TRUE, auto_unbox = TRUE)
