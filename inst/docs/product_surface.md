...
| LMM | `ds.vertLMM()` K=2 closed form | `ds.vertLMM()` dispatches to the K>=3 variance-ratio profile (`ds.vertLMM.k3()` deprecated) | Guarded cluster-size/cluster-summary disclosure only | DSLite-validated against centralized mixed-model targets in the current evidence set | Client-supplied cluster-vector helper as product route |
...
| GLMM | `ds.vertGLMM()` PQL through the `ds.vert.glmm()` frontdoor | Same PQL route | Share-domain cluster sufficient statistics; no BLUP vector, row probability, row score, or cluster vector returned | PQL is the validated product route. The internal Laplace development route targets `glmer(nAGQ = 1)` but is not part of the current validated frontdoor surface. | Legacy EM, Laplace frontdoor exposure and patient-level BLUP/probability routes removed |
| IPW | `ds.vertIPW()` | `ds.vertIPW()` | Propensity and outcome GLM aggregates only; the IPW column is consumed server-side and not returned | Covered by DSLite method evidence using a server-side weight column | Returning per-patient propensity scores or weights |
