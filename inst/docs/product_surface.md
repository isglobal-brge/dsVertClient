...
| GLMM | `ds.vertGLMM()` PQL and `ds.vertGLMMLaplace()` / `ds.vert.glmer()` Laplace | Same PQL and Laplace routes | Share-domain cluster sufficient statistics; no BLUP vector, row probability, row score, or cluster vector returned | PQL is the supported fast approximation; Laplace is the supported practical route against the `glmer(nAGQ = 1)` target | Legacy EM and patient-level BLUP/probability routes removed |
