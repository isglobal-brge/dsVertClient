...
| `ds.vertDPStatus()` | Read-only joint-DP capsule-status v6/accounting handshake | All peers must agree on domain, fixed per-capsule privacy parameters, `N`, adjacency, complete pinset, designated peers and one exact `jdpc1_` accountant namespace protected by an identity-bound immutable receipt; allocator-commit history can deny a new capsule at `N`, while `request_limit` is false and exact replay remains free |
