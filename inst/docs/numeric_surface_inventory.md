...
| `ds.vertLMM`, `ds.vertLMM.k3` | Signed same-owner random-intercept Synopsis moments | Promoted `outcome ~ 1` moment post-processing, including K>=3 compatibility | ML/REML, random slopes, covariance, standard errors and sampling inference remain unavailable. |
...
| `ds.vertMI` | Signed sticky categorical Synopsis with strict missingness | Formal sticky Synopsis route with deterministic categorical MCAR completion and no analyst seed | This is intercept-only categorical MCAR post-processing; it returns neither source rows nor Rubin/sampling inference. Covariate imputation and MAR/MNAR models remain unavailable. |
