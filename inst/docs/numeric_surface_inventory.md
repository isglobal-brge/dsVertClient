...
| `ds.vertGLMM` | Signed binary same-owner `outcome ~ 1` random-intercept moments, same-owner finite binary marginal-likelihood grid, or same-owner finite bounded-count Poisson random-intercept/one-random-slope grid | Population-average log-odds/observed-scale ICC moment approximation, additive binary bare-covariate finite random-intercept/one-to-three-random-slope selection, or additive Poisson random-intercept/one-random-slope selection | PQL/Laplace/ML/REML, interactions, more than one Poisson or three binary random slopes, covariance and sampling inference remain unavailable. |
