...
| `ds.vertGEE` | Signed finite binomial/Poisson grid or completed formal GLM certificate, or matching signed Gaussian random-intercept Synopsis | Read-only independence point adapter, or Gaussian exchangeable model-based working GLS | The independent score equals the corresponding GLM score; the Gaussian random-intercept covariance has the same exchangeable working form. Fresh formal computation, cluster statistics, robust covariance, standard errors and inference are unavailable; AR(1) still requires a protected cluster artifact. |
| `ds.vertGLMM` | Signed binary same-owner `outcome ~ 1` random-intercept moments, or same-owner finite marginal-likelihood grid | Population-average log-odds/observed-scale ICC moment approximation, or additive bare-covariate finite random-intercept/one-or-two-random-slope selection | PQL/Laplace/ML/REML, interactions, more than two random slopes, covariance and sampling inference remain unavailable. |
