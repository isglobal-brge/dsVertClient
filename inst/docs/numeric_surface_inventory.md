...
| `ds.vertGEE` | Signed finite binomial/Poisson grid or completed formal GLM certificate, matching signed Gaussian random-intercept Synopsis, a signed Gaussian AR(1) beta/rho working-GLS grid, or a distinct bounded AR(1) bread/meat grid with strict within-cluster order | Read-only independence point adapter, Gaussian exchangeable model-based working GLS, Gaussian AR(1) working GLS, or componentwise-clipped cluster-score sandwich covariance | The independent score equals the corresponding GLM score; the Gaussian random-intercept covariance has the same exchangeable working form. The bread/meat route has at most three additive predictors, public cluster levels and score clipping; it emits covariance only when DP bread is positive definite. Fresh formal computation, binomial/Poisson robust GEE, standard errors, p-values and sampling inference remain unavailable. |
