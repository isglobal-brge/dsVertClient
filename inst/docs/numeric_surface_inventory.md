...
| `ds.vertCoxDiscreteNonDisclosive` | Ring127 pooled-logistic hazard route | Compatibility estimand, unattested | Same arithmetic work as Cox plus keep the discrete-time estimand label explicit. |
...
| `ds.vertGEE` | GLM seed plus Ring127 cluster score/sandwich work | Quarantine, unattested | Whole-GEE adapter; do not inherit a seed GLM certificate. Move granular cluster information fully into protected aggregation and certify the sandwich. |
...
| `ds.vertLASSOProximal` | Client Gaussian normal-equation reconstruction and proximal solver | Provisional, inherits input | Condition-number/PSD and convergence certificate for the reconstructed objective; no silent curvature substitution. |
