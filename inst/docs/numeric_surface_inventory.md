...
| `ds.vertMultinom` | Validated sticky categorical Frequency release | Intercept-only log-odds post-processing with deterministic Jeffreys smoothing | This is `y ~ 1` only; it inherits the Frequency DP contract and returns no covariance or inference. `ds.vertMultinomJointNewton` remains quarantined until a signed `multinomial_design_grams` workload binds the exact score design. |
| `ds.vertOrdinal` | Validated sticky categorical Frequency release | Intercept-only cumulative-logit thresholds with deterministic Jeffreys smoothing | The caller supplies a complete order matching the signed category domain. This is `y ~ 1` only and returns no covariance or inference. `ds.vertOrdinalJointNewton` remains quarantined pending a protected ordinal score/information artifact. |
