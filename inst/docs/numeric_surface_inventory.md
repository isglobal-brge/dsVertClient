...
| `ds.vertCoxProfileNonDisclosive`, `ds.vertCox` | Same-owner signed bounded Breslow partial-likelihood finite grid or completed two-authority formal Cox certificate | Sticky finite-grid selection or read-only certificate adapter | No arbitrary optimizer, covariance, standard errors, p-values, baseline hazards, predictions, residuals, strata, delayed entry or sampling inference. |
...
| `ds.vertGEE` | Signed finite binomial/Poisson grid or completed formal GLM certificate, matching signed Gaussian random-intercept Synopsis, signed Gaussian AR(1) beta/rho/bread/meat grids, or same-owner binomial/Poisson robust-independence bread/meat grids | Read-only independence point adapter, Gaussian exchangeable model-based working GLS, Gaussian AR(1) working GLS, or componentwise-clipped cluster-score sandwich covariance | The independent score equals the corresponding GLM score; the Gaussian random-intercept covariance has the same exchangeable working form. Robust grids accept at most three additive predictors, public cluster levels and score clipping; covariance is exposed only when DP bread is positive definite, otherwise a non-identifiability status and no covariance are returned. No standard errors, p-values or sampling inference. |
| `ds.vertGLMM` | Signed binary same-owner `outcome ~ 1` random-intercept moments, same-owner finite binary marginal-likelihood grid, or same-owner finite bounded-count Poisson random-intercept/one-to-three-random-slope grid | Population-average log-odds/observed-scale ICC moment approximation, additive binary or Poisson bare-covariate finite random-intercept/one-to-three-random-slope selection | PQL/Laplace/ML/REML, interactions, more than three random slopes, covariance and sampling inference remain unavailable. |
...
| `ds.vertLASSOIter` | One signed same-owner Gaussian Synopsis moment artifact, or a signed finite binomial/Poisson L1 candidate path | Promoted; Gaussian is deterministic KKT-certified post-processing, while binomial/Poisson release bounded negative log likelihood for every signed beta/lambda candidate and add the signed capacity-normalized L1 penalty client-side | No sampling inference, coefficient regions, cross-owner design or analyst-generated path. The legacy protected score/Hessian optimiser remains quarantined. |
