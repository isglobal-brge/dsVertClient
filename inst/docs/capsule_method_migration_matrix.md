...
| GLM: `ds.vertGLM`, `ds.vert.glm` | admitted count, bounded moments/cross-products, score, Hessian and canonical deviance; convergence, identifiability and DP-aware covariance | an explicit compatible Gaussian `dp_analysis_id` delegates to the same- or cross-owner `ds.vertDPGaussian` capsule; default calls and binomial/Poisson variants still require migration |
...
| Cox dispatcher/discrete hazard: `ds.vert.cox`, `ds.vert.coxph`, `ds.vertCoxDiscreteNonDisclosive` | Cox artifacts above or a separately labelled fixed-grid pooled-logistic hazard artifact | new artifact; grid-based hazard must never be relabelled as an exact Cox estimand |
| Negative binomial: `ds.vertNBFullRegTheta`, `ds.vert.nb` | NB2 log-link beta score/information, theta score/information, beta-theta cross-information, deviance and joint covariance | new secure protocol and joint beta/theta inference required |
| Multinomial: `ds.vertMultinom`, `ds.vertMultinomJoint`, `ds.vertMultinomJointNewton`, `ds.vert.multinom` | fixed outcome domain, joint softmax score/Hessian, and a signed `multinomial_design_grams` artifact bound to the exact raw score design (snapshot, complete-case mask, scaling and coefficient order) | slope-bearing requests now fail closed before DSI; the legacy local-moment/correlation reconstruction was removed and the purpose-bound server artifact is not yet materialized |
| Ordinal: `ds.vertOrdinal`, `ds.vertOrdinalJointNewton`, `ds.vert.ordinal` | ordered outcome domain, bounded cross-products and proportional-odds score/Hessian; diagnostics | new capsule backend required |
| LMM: `ds.vertLMM`, `ds.vert.lmm` | contribution-bounded cluster moments/cross-products, random-effects design and LMM score/information; ML/REML contract | current granular route is quarantined; new secure protocol required |
| Deprecated LMM K>=3 subroute: `ds.vertLMM.k3` | same LMM contract plus an explicit K>=3 restriction | `deprecated_subroute`, not an alias of `ds.vertLMM`; new secure protocol required |
| GEE: `ds.vertGEE`, `ds.vert.gee` | contribution-bounded cluster score/meat and working-correlation artifact; robust DP-aware covariance | current granular route is quarantined; new secure protocol required |
| GLMM: `ds.vertGLMM`, `ds.vert.glmm` | bounded cluster moments, working score/information and validated target estimator | current granular route is quarantined; new secure protocol required |
| IPW: `ds.vertIPW`, `ds.vert.ipw` | treatment/outcome binding, propensity and outcome score/information, bounded weight distribution, ATE/ATT/ATC choice, stabilization/clipping and weight provenance | current route does not complete this estimand contract; new secure protocol required |
| Multiple imputation: `ds.vertMI`, `ds.vert.mi` | bounded missingness counts, imputation sufficient statistics, posterior parameter draws and synthetic draws from a non-rerollable cryptographic stream | current mutating route and inference need redesign; Rubin small-sample degrees of freedom must be used |
...
| Iterative LASSO: `ds.vertLASSOIter`, `ds.vert.lasso_iter` | bounded moments/cross-products, score/Hessian and deviance; a signed `binomial_lasso_design_grams` majorant bound to the exact score design; KKT and DP-aware model-selection contract | exact slope-binomial requests now fail closed before DSI because the Gaussian correlation capsule has a different clipped/normalised design; purpose-bound server artifact and whole-path capsule backend required |
...
| Wald inference: `ds.vertConfint`, `ds.vert.confint`, `ds.vertWald`, `ds.vert.wald`, `ds.vertContrast`, `ds.vert.contrast` | attested coefficients and valid compatible covariance | client algebra exists and inherits input limitations |
