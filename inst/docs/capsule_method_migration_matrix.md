...
| IPW: `ds.vertIPW`, `ds.vert.ipw` | a signed binary treatment-by-outcome table for `outcome ~ treatment`, `treatment ~ 1`, or a signed categorical stratum-treatment-by-outcome table with complete public row mapping and fixed target weights for `treatment ~ stratum` | the exact intercept-only or one-categorical-stratum saturated IPW/g-formula ATE identity is available without releasing weights or a propensity fit. ATT/ATC, continuous/multiple covariates, stabilization/clipping and outcome regression still require a purpose-bound secure protocol |
