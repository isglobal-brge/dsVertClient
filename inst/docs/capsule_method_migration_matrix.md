...
| Negative binomial: `ds.vertNBFullRegTheta`, `ds.vert.nb` | One validated sticky bounded non-negative integer Frequency release for an intercept-only log-mean and NB2 method-of-moments dispersion | `y ~ 1` post-processing is available without a new DSI call; covariate NB2, likelihood optimization, covariance and inference still require protected joint beta/theta score/information |
| Multinomial: `ds.vertMultinom`, `ds.vertMultinomJoint`, `ds.vertMultinomJointNewton`, `ds.vert.multinom` | one validated sticky categorical Frequency release for `y ~ 1`; slopes additionally need a signed `multinomial_design_grams` artifact bound to the exact raw score design (snapshot, complete-case mask, scaling and coefficient order) | all listed names safely post-process the Frequency release for intercept-only log-odds; slope-bearing requests fail closed before DSI and the unsafe local-moment/correlation reconstruction remains removed |
| Ordinal: `ds.vertOrdinal`, `ds.vertOrdinalJointNewton`, `ds.vert.ordinal` | one validated sticky categorical Frequency release and a complete clinical order for `y ~ 1`; covariates additionally need bounded cross-products and a proportional-odds score/Hessian | all listed names safely post-process the Frequency release for thresholds; covariate proportional-odds requests require a new protected score/information backend |
| LMM: `ds.vertLMM`, `ds.vert.lmm` | one signed same-owner `outcome ~ 1` random-intercept moment artifact with bounded cluster contributions | validated sticky Synopsis post-processing; no ML/REML, random slopes, covariance, standard errors or sampling inference |
| LMM K>=3 compatibility: `ds.vertLMM.k3` | same signed moment artifact and at least three peers | compatibility wrapper, tested through K=5; no profile/REML route |
...
| GLMM: `ds.vertGLMM`, `ds.vert.glmm` | one signed same-owner binary `outcome ~ 1` random-intercept moment artifact | validated sticky Synopsis post-processing for population-average log-odds and observed-scale ICC; PQL/Laplace/ML/REML, covariates and inference remain unavailable |
| IPW: `ds.vertIPW`, `ds.vert.ipw` | the implemented subcase is a signed binary treatment-by-outcome table with `outcome ~ treatment`, `treatment ~ 1`, an explicit treated level and conservative DP-plus-sampling regions | the exact intercept-only ATE IPW identity is available without releasing weights or a propensity fit. Covariate IPW, ATT/ATC, stabilization/clipping and outcome regression still require a purpose-bound secure protocol |
