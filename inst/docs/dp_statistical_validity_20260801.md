...
| `ds.vertDPFrequency()` | Fixed-domain marginal counts and proportions from the canonical PSI-v4 aligned source snapshot | At most the first eligible source record per privacy unit; missing or out-of-domain factor values contribute zero | One authenticated non-negative sticky Ring128 vector plus simultaneous count and proportion mechanism regions | Base regions exclude population sampling uncertainty; source ordering and factor-domain provisioning are part of the signed estimand |
...
- Survival times are clipped to public limits and mapped to the first upper
  ...
  `ds.vertDPLogRank()` combines two compatible releases into an all-cause or
  cause-specific fixed-grid score. Its outer region propagates the simultaneous
  histogram coordinate boxes and uses Bonferroni for distinct releases; its
  null variance and standardised score are descriptive plug-in diagnostics,
  not a test, p-value, standard error or sampling interval. Authenticated
  release provenance does not itself establish that the two declared cohorts
  are disjoint.
