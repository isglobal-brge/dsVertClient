...
We connect to the three Opal servers hosting the vertically partitioned data
and perform ECDH-PSI record alignment. Each server only knows its own patient
identifiers; the PSI protocol finds the common cohort without revealing which
IDs are shared.

```{r connect, results = "hide", message = FALSE, cache = FALSE}
library(DSI)
library(DSOpal)
...
```{r psi, results = "hide", message = FALSE, cache = FALSE}
...
```{r dsvert-cor, results = "hide", message = FALSE}
variables <- list(
...
```{r dsvert-gauss, results = "hide", message = FALSE}
...
```{r dsvert-binom, results = "hide", message = FALSE}
...
```{r dsvert-pois, results = "hide", message = FALSE}
glm_pois <- ds.vertGLM(
...
legend(par("usr")[2], par("usr")[4], legend = c("Centralized", "dsVert"),
...
```{r disconnect, include = FALSE, cache = FALSE}
