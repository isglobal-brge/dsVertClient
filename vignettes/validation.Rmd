...
cache_dir <- file.path(getwd(), "validation_cache")
...
full protocol stack --- ECDH-PSI record alignment, Ring63-Beaver correlation,
...
files. Re-rendering the vignette loads cached results instantly. Delete the
`validation_cache/` directory to force re-execution.
...
We load the dataset from `MASS::Pima.tr`, assign patient identifiers, and
partition it into three vertical fragments with **deliberately non-identical
patient populations**. This simulates a realistic scenario where institutions
have data on overlapping but non-identical patient cohorts.
...
These are the fragments that each Opal server contains. Row order is
independently shuffled, so the ECDH-PSI alignment protocol has real work to
do --- it must identify the common cohort without revealing which IDs are
shared or which patients are unique to each server.

| Server  | Patients | Table (vert\_demo) | Variables                         |
|---------|----------|--------------------|-----------------------------------|
| server1 | 180      | pima\_server1      | `patient_id`, `age`, `bmi`, `ped` |
| server2 | 175      | pima\_server2      | `patient_id`, `glu`, `bp`, `skin` |
| server3 | 170      | pima\_server3      | `patient_id`, `npreg`, `diabetes` |

---

## Connect to DataSHIELD and align records

We connect to three DataSHIELD sessions on the OBiBa community Opal server
([opal-demo.obiba.org](https://opal-demo.obiba.org)), each pointing to a
different vertical partition of the dataset stored in the `vert_demo` project.
This demonstrates that dsVert works on a real, publicly accessible Opal
deployment --- not just a local Docker setup.
...
for (i in 1:3) {
  builder$append(server = paste0("server", i),
                 ...
                 driver = "OpalDriver",
                 table = paste0("vert_demo.pima_server", i))
}
...
We reconstruct the ground truth by merging the three server fragments on
`patient_id` --- only patients present in all three servers are kept. This
is exactly the cohort that the PSI protocol should have identified.
...
from multiple servers) are computed under Ring63 MPC
(Ring63-Beaver) with secret-shared cross-products, so individual-level data
never leaves any server.

### Correlation (Ring63-Beaver)
...
Ring63 Beaver cross-products and server-side fusion.
...
heatmap(dsvert_cor, symm = TRUE, col = col_palette, margins = c(8, 8),
        Rowv = NA, Colv = NA,
        main = "dsVert Federated Pearson Correlation (Ring63-Beaver)")
```
...
distributed across all three servers. Gradients are computed under Ring63-Beaver
MPC and reconstructed via share-wrapped secret sharing with server-side fusion.
...
**Correlation and PCA** ($\sim 10^{-6}$). These errors arise from Ring63
fixed-point arithmetic used in the Beaver MPC protocol. Values are encoded
as 63-bit ring integers with a configurable fractional precision, providing
approximately $2^{40} \approx 10^{12}$ precision units. After encoding,
Beaver triple multiplication, inner-sum accumulation, and secret-share
reconstruction, the effective precision degrades to $\sim 10^{-6}$,
consistent with fixed-point error propagation theory.
