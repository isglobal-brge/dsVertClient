...
The supported route is joint proportional-odds Newton over Ring127 shares. Historical cumulative-binomial output is internal warm start only.

## Mathematical target

P(Y <= k | X)=sigmoid(theta_k - X beta). The validation compares cumulative probabilities to MASS::polr().
...
```{r evidence}
rows <- run_validation("ordinal", force = force_run, trace = TRUE)
