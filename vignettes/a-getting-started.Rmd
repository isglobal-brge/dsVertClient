...
Records across servers are stored in different row orders. `ds.hashId()`
retrieves SHA-256 hashes of the identifiers from a reference server, and
`ds.alignRecords()` uses those hashes to reorder every server so that the
same row position corresponds to the same patient. The original identifiers
never leave their respective servers.
