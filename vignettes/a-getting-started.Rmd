...
In a vertically partitioned setting, different institutions hold different
variables for the same set of patients. A shared patient identifier links
records across sites, but no single institution has access to the complete
variable set. dsVertClient extends DataSHIELD to run privacy-preserving
statistical methods across these disjoint columns without ever centralising
the raw data.
...
Build a login data frame containing the URL, credentials, and table reference
for each Opal server, then call `datashield.login()` to open connections and
assign each table to a server-side symbol called `"D"`.
...
Before aligning records, confirm that the patient identifiers are present,
unique, and consistently formatted at every server. `ds.validateIdFormat()`
...
Records across servers are stored in different row orders. `ds.psiAlign()`
uses an ECDH-based Private Set Intersection (PSI) protocol to determine the
common set of patient identifiers and reorder every server so that the same
row position corresponds to the same patient. Each server masks its
identifiers with a secret elliptic-curve scalar; the client sees only opaque
curve points and cannot recover the original identifiers.
...
With records aligned, compute a cross-server correlation matrix to verify that
the data is accessible and the alignment is correct. The analyst specifies
which variables reside on which server.
...
Always close the server connections when the analysis session is finished.
...
- **Statistical Analysis** -- correlation, PCA, and GLM examples with
  interpretation guidance: `vignette("b-statistical-analysis")`
- **Methodology** -- mathematical foundations of Block Coordinate Descent,
  MHE threshold decryption, and the security model:
  `vignette("c-methodology")`
- **Validation** -- empirical validation of accuracy guarantees:
  `vignette("d-validation")`
