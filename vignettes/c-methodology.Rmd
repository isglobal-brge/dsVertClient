...
| SHA-256 Hashing | `ds.hashId()` + `ds.alignRecords()` | Privacy-preserving record linkage |
...
The encrypted product is then threshold-decrypted: each party $k$ computes a
partial decryption share $\text{share}_k = -sk_k \cdot ct[1] + e_k'$ (where
$e_k'$ is a smudging noise term). The client fuses all shares:

$$m \approx ct[0] + \sum_{k=1}^{K} \text{share}_k$$

**Phase 6.** The client sums the first $n$ slots of the recovered plaintext to
obtain the inner product, then divides by $(n - 1)$:
...
1. The label server encrypts $y$ under the CPK, producing $ct_y$. This
   ciphertext is distributed to all non-label servers.
2. $ct_y$ is distributed to non-label servers via a chunked transfer protocol.
3. A non-label server $k$ computes: $ct_u = ct_y - \text{encode}(\mu)$, then
   $g_k[j] = \text{InnerSum}(\text{encode}(x_j) \cdot ct_u)$ for each feature
   $j$. The InnerSum operation uses Galois rotations to sum all $n$ SIMD slots
   into a single scalar, collapsing $n$ individual-level products into one
   aggregate inside the encrypted domain.
4. Threshold decrypt $g_k$ (length $p_k$). Each of the $K$ servers provides a
...
The label server uses standard IRLS with plaintext $y$; no encryption is needed
for its own block.
...
SHA-256 hashing of patient identifiers for privacy-preserving record linkage.

SHA-256 has three properties that make it suitable:

- **One-way.** Given a hash, it is computationally infeasible to recover the
  original identifier.
- **Deterministic.** The same identifier always produces the same 256-bit hash,
  so matching across servers is exact.
- **Collision-resistant.** The probability that two different identifiers
  produce the same hash is negligible ($< 2^{-128}$).

The protocol is straightforward: the reference server's hash order defines the
canonical row ordering, and every other server reorders its data frame to match.
The original identifiers never leave their respective servers; only the
irreversible hashes are transmitted.
