...
All methods share a key principle: **decompose computations so each institution only sees its own data, while sharing only aggregate statistics**.
...
where institution $k$ holds $X_k$ (its subset of columns), we want to compute:
...
1. Each institution $k$ computes SVD of its own data: $X_k = U_k D_k V_k^T$
...
- It has the same number of rows as observations but typically far fewer columns
...
1. Stack the partial results: $M = [U_1 D_1 | U_2 D_2 | ... | U_K D_K]$
2. Compute SVD of $M$: $M = \tilde{U} \tilde{D} \tilde{V}^T$
3. The correlation matrix is: $R = \tilde{V} \tilde{D}^2 \tilde{V}^T / (n-1)$
...
1. **Block SVD**: Combines partial SVD results to compute global correlation/PCA
