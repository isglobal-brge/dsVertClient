...
The product route is aggregate PQL for a binomial random-intercept GLMM. The compact executable vignette runs one protected PQL outer update and checks that the fixed-effect symmetry target and variance trace are finite.

## Mathematical target

A PQL step alternates protected binomial working-response updates with aggregate weighted mixed-model normal equations. The validation compares fixed effects to the central symmetric glm target and asserts that the PQL trace is populated.

## Fixture and reference

Fixture: Paired balanced binomial cluster fixture with cluster sizes above privacy thresholds.

Centralized reference: Central binomial glm fixed-effect symmetry plus a one-step PQL trace check.
