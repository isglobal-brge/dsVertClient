...
The strongest validation of the MHE protocol is to perform the **exact same
analysis** in two scenarios and compare the results:

- **Scenario 1 (Local)**: Pool all the raw data in one place and run standard
  R functions (`cor()`, `eigen()`).
- **Scenario 2 (Opal)**: Data stays distributed across 3 servers. Use
  `ds.vertCor()` and `ds.vertPCA()` with MHE threshold decryption.

If the MHE protocol is correct, both scenarios should produce the same
correlation matrix and the same PCA results (eigenvalues, loadings, variance
explained), up to the small CKKS approximation error.

### Scenario 1: Local Analysis (Ground Truth)

If we had access to the raw data (which in a real study we would not), we
could compute the correlation and PCA directly:

```{r local-analysis, purl = FALSE}
...
Note the use of `cor_result = cor_result` in `ds.vertPCA()`. This reuses
the correlation matrix already computed by `ds.vertCor()`, avoiding a second
expensive MHE protocol run. Since PCA is simply the eigen decomposition of
the correlation matrix, the result is mathematically identical.
...
```{r compare-correlation, purl = FALSE}
...
explained by the CKKS approximation. CKKS is an *approximate* homomorphic
encryption scheme: it encodes real numbers with a fixed number of bits of
precision controlled by `log_scale`. With the default `log_scale = 40`, we
get approximately 12 decimal digits of precision for individual operations,
but the cross-product accumulates rounding across n additions, resulting in
final errors around 10^-3 to 10^-4 for n = 200.
...
Since PCA is computed from the correlation matrix via eigen decomposition
(a deterministic operation), the PCA differences are even smaller --
they are a second-order effect of the correlation differences.
