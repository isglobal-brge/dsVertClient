...
.DSVERT_CLIENT_DP_GROUPED_GRID_CROSS_FAMILIES <- c(
  "lmm", "binomial_glmm", "poisson_glmm", "binomial_gee", "poisson_gee")
.DSVERT_CLIENT_DP_GROUPED_GRID_CROSS_VERSION <- "dsvert-grouped-cross-signed-contract-v1"
.DSVERT_CLIENT_DP_GROUPED_GRID_CROSS_DOMAIN <- "dsVert/grouped-cross-contract/v1|"

.dsvert_dp_grouped_grid_cross_fail <- function() .dsvert_dp_glm_grid_cross_fail()

.dsvert_dp_grouped_grid_cross_scalar <- function(value, lower, upper) {
...
.dsvert_dp_grouped_grid_cross_parameters <- function(value, family) {
  if (identical(family, "lmm") && "objective" %in% names(value)) {
    return(.dsvert_dp_grouped_grid_cross_ml_parameters(value))
  }
  if (family %in% c("binomial_glmm", "poisson_glmm") && "variance_grid" %in% names(value)) {
    ...
    if (!is.list(value$variance_grid) || !is.null(names(value$variance_grid)) ||
        ...
        !identical(value$quadrature, "gh5_fixed_v1")) .dsvert_dp_grouped_grid_cross_fail()
    grid <- lapply(value$variance_grid, function(tau) {
      tau <- .dsvert_dp_grouped_grid_cross_scalar(tau, 0, .25)
      if (!tau %in% c(0, .25)) .dsvert_dp_grouped_grid_cross_fail()
      tau
    })
    if (any(diff(unlist(grid, use.names = FALSE)) <= 0)) .dsvert_dp_grouped_grid_cross_fail()
  ...
  if (staged_gee) {
    if (!identical(value$composition, "staged_fixed_rho_v1")) .dsvert_dp_grouped_grid_cross_fail()
    fields <- c(fields, "composition")
  }
  .dsvert_dp_glm_grid_cross_fields(value, fields)
  if (family == "lmm") {
    sigma <- .dsvert_dp_grouped_grid_cross_scalar(value$residual_variance, .25, 16)
    tau <- .dsvert_dp_grouped_grid_cross_scalar(value$random_intercept_variance, 0, 16)
  ...
  if (grepl("_glmm$", family)) {
    tau <- .dsvert_dp_grouped_grid_cross_scalar(value$random_intercept_variance, 0, .25)
  ...
  rho <- .dsvert_dp_grouped_grid_cross_scalar(value$rho, 0, .5)
  if (!rho %in% c(0, .25, .5) ||
      (value$correlation == "independence" && rho != 0)) {
    .dsvert_dp_grouped_grid_cross_fail()
  }
  clip <- .dsvert_dp_grouped_grid_cross_scalar(value$score_clip, .25, 4)
  if (clip * 4 != floor(clip * 4)) .dsvert_dp_grouped_grid_cross_fail()
...
.dsvert_dp_grouped_grid_cross_sensitivity <- function(
  ...
  if (identical(family, "lmm") && identical(parameters$objective, "ml")) {
    return(.dsvert_dp_grouped_grid_cross_ml_sensitivity(beta_grid, grid_bits, grouping,
  ...
  bounds <- if (family %in% c("binomial_glmm", "poisson_glmm") && !is.null(parameters$variance_grid)) {
    unlist(lapply(parameters$variance_grid, function(tau) {
      .dsvert_dp_grouped_grid_cross_sensitivity(beta_grid, family, max_outcome,
  ...
  if (any(!is.finite(c(caps, C*caps, l1, l2))) ||
      any(c(caps, C*caps, l1, l2) > 2^53-1)) .dsvert_dp_grouped_grid_cross_fail()
...
.dsvert_dp_grouped_grid_gee_errors <- function(family, B, clip) {
...
.dsvert_dp_grouped_grid_cross_numeric <- function(family, grouping, parameters) {
  if (identical(family, "lmm") && identical(parameters$objective, "ml")) {
    return(.dsvert_dp_grouped_grid_cross_ml_numeric(grouping, parameters))
  }
  B <- grouping$max_patients_per_cluster
  lmm <- identical(family, "lmm")
  gee <- grepl("_gee$", family)
  errors <- if (gee) .dsvert_dp_grouped_grid_gee_errors(family, B, parameters$score_clip) else NULL
  ...
  if (gee) {
    ...
    if (identical(parameters$composition, "staged_fixed_rho_v1")) {
      ...
      paths <- system.file("certificates", c("grouped_gee_fixed_rho_staged_v1.json",
        "grouped_pwlinear_q16_v1.json"), package = "dsVertClient")
  ...
  if (family %in% c("binomial_glmm", "poisson_glmm") && !is.null(parameters$variance_grid)) {
    ...
    if (error >= out$per_cluster_error_bound) .dsvert_dp_grouped_grid_cross_fail()
    out$certificate_sha256 <- certificate
...
.dsvert_dp_grouped_grid_cross_spec <- function(raw, policy, schema) {
  ...
  if (length(family) != 1L || !family %in% .DSVERT_CLIENT_DP_GROUPED_GRID_CROSS_FAMILIES ||
  ...
  if (base$observation_capacity > B*C ||
      length(columns[[which(match)]]$levels) > C) .dsvert_dp_grouped_grid_cross_fail()
  ...
  parameters <- .dsvert_dp_grouped_grid_cross_parameters(raw$parameters, family)
  if (C > 64) {
  ...
  if (grepl("_gee$", family) && identical(parameters$composition, "staged_fixed_rho_v1")) {
    if (length(base$beta_grid) > 4L) .dsvert_dp_grouped_grid_cross_fail()
    for (encoded in base$beta_encoded) {
      beta <- as.numeric(unlist(encoded, use.names = FALSE))
      if (beta[1L] + sum(pmin(beta[-1L], 0)) < -4*2^50 ||
          beta[1L] + sum(pmax(beta[-1L], 0)) > 4*2^50) .dsvert_dp_grouped_grid_cross_fail()
    }
  }
  ...
  base$numeric_contract <- .dsvert_dp_grouped_grid_cross_numeric(family, grouping, parameters)
...
.dsvert_dp_grouped_grid_cross_spec_validate <- function(value, policy, schema) {
  tryCatch({
    ...
    expected <- .dsvert_dp_grouped_grid_cross_spec(raw, policy, schema)
    .dsvert_dp_glm_grid_cross_equal(value, expected)
    expected
  }, error = .dsvert_dp_glm_grid_cross_transcript_stop)
}

.dsvert_dp_grouped_grid_cross_layout <- function(spec) {
  base <- spec
  ...
  layout$grouping_controls <- list(
    ...
    source_encoding_sha256 = .dsvert_dp_capsule_source_hash(routing),
...
.dsvert_dp_grouped_grid_cross_artifact <- function(spec) {
  out <- list(version = paste0("bounded-", gsub("_", "-", spec$family), "-cross-grid-v1"),
       spec_version = spec$version, spec_sha256 = .dsvert_dp_capsule_source_hash(spec),
       ...
       numeric_contract_sha256 = .dsvert_dp_capsule_source_hash(spec$numeric_contract),
       private_layout_sha256 = .dsvert_dp_capsule_source_hash(.dsvert_dp_grouped_grid_cross_layout(spec)),
...
.dsvert_dp_grouped_grid_cross_source_contract <- function(spec, artifact) {
  list(version = "dsvert-grouped-cross-private-source-contract-v1",
       ...
       spec_sha256 = .dsvert_dp_capsule_source_hash(spec),
       ...
       private_layout = .dsvert_dp_grouped_grid_cross_layout(spec))
}

.dsvert_dp_grouped_grid_cross_message <- function(unsigned) {
  charToRaw(paste0(.DSVERT_CLIENT_DP_GROUPED_GRID_CROSS_DOMAIN,
    .dsvert_joint_dp_client_json(.dsvert_joint_dp_client_canonical(unsigned))))
}

.dsvert_dp_grouped_grid_cross_contract_validate <- function(
  ...
  tryCatch({
    ...
    if (!identical(value$version, .DSVERT_CLIENT_DP_GROUPED_GRID_CROSS_VERSION)) {
      .dsvert_dp_grouped_grid_cross_fail()
    }
    schema <- .dsvert_dp_glm_grid_cross_schema_validate(
      policy, schema_manifest$logical_snapshot, schema_manifest, .verifier)
    spec <- .dsvert_dp_grouped_grid_cross_spec_validate(value$spec, policy, schema)
    ...
    .dsvert_dp_glm_grid_cross_equal(value$source_contract,
      .dsvert_dp_grouped_grid_cross_source_contract(spec, artifact))
    ...
    message <- .dsvert_dp_grouped_grid_cross_message(unsigned)
    ...
    if (!all(verified)) .dsvert_dp_grouped_grid_cross_fail()
    .dsvert_joint_dp_client_canonical(value)
  }, error = .dsvert_dp_glm_grid_cross_transcript_stop)
}

.DSVERT_CLIENT_DP_GROUPED_GRID_CROSS_SPEC_VERSIONS <- stats::setNames(
  paste0(.DSVERT_CLIENT_DP_GROUPED_GRID_CROSS_FAMILIES, "_grid_cross_v1"),
  .DSVERT_CLIENT_DP_GROUPED_GRID_CROSS_FAMILIES)

.dsvert_dp_grouped_grid_cross_artifact_validate <- function(value, spec) {
  tryCatch({
    expected <- .dsvert_dp_grouped_grid_cross_artifact(spec)
    .dsvert_dp_glm_grid_cross_equal(value, expected)
    expected
  }, error = .dsvert_dp_glm_grid_cross_transcript_stop)
}
