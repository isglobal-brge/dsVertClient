...
.DSVERT_CLIENT_DP_COX_GRID_CROSS_SPEC_VERSION <- "cox_grid_cross_v1"
.DSVERT_CLIENT_DP_COX_GRID_CROSS_ARTIFACT_VERSION <-
  "bounded-cox-breslow-observed-cross-grid-v1"
.DSVERT_CLIENT_DP_COX_GRID_CROSS_CONTRACT_VERSION <-
  "dsvert-cox-cross-owner-grid-signed-contract-v1"
.DSVERT_CLIENT_DP_COX_GRID_CROSS_SIGNING_DOMAIN <-
  "dsVert/cox-cross-owner-grid-contract/v1|"

.dsvert_dp_cox_grid_cross_fail <- function() {
  .dsvert_dp_glm_grid_cross_transcript_stop(simpleError("Invalid Cox cross-grid contract."))
}
...
.dsvert_dp_cox_grid_cross_numeric <- function() {
  ...
  # Public numeric identities are pinned; the client never evaluates a loss.
...
.dsvert_dp_cox_grid_cross_spec <- function(raw, policy, schema) {
  tryCatch({
    ...
    if (!identical(raw$version, .DSVERT_CLIENT_DP_COX_GRID_CROSS_SPEC_VERSION)) {
      .dsvert_dp_cox_grid_cross_fail()
    }
    ...
    descriptors <- lapply(variables, function(variable) {
      ...
      .dsvert_dp_glm_grid_cross_fields(column, c("kind", "owner_peer", "lower", "upper"))
      if (!identical(column$kind, "numeric") ||
          ...
          !is.finite((column$upper - column$lower)^2)) {
        .dsvert_dp_cox_grid_cross_fail()
      }
    ...
    keys <- vapply(beta, .dsvert_joint_dp_client_json, character(1L))
    ...
    if (anyDuplicated(keys) || !identical(keys, sort(keys, method = "radix")) ||
        anyDuplicated(vapply(encoded, .dsvert_joint_dp_client_json, character(1L)))) {
      .dsvert_dp_cox_grid_cross_fail()
    }
    ...
    list(version = raw$version, family = "cox", analysis_id = raw$analysis_id,
      ...
      candidate_order = as.list(vapply(beta, .dsvert_dp_capsule_source_hash, character(1L))),
...
.dsvert_dp_cox_grid_cross_artifact <- function(spec) {
  list(version = .DSVERT_CLIENT_DP_COX_GRID_CROSS_ARTIFACT_VERSION,
    ...
    private_layout_sha256 = .dsvert_dp_capsule_source_hash(.dsvert_dp_cox_grid_cross_layout(spec)),
...
.dsvert_dp_cox_grid_cross_source_contract <- function(spec, artifact) {
  list(version = "dsvert-cox-cross-owner-source-contract-v1",
    ...
    spec_sha256 = .dsvert_dp_capsule_source_hash(spec), artifact_sha256 = .dsvert_dp_capsule_source_hash(artifact),
...
.dsvert_dp_cox_grid_cross_message <- function(unsigned) {
  charToRaw(paste0(.DSVERT_CLIENT_DP_COX_GRID_CROSS_SIGNING_DOMAIN,
...
.dsvert_dp_cox_grid_cross_contract_validate <- function(
  ...
  tryCatch({
    ...
    if (!identical(value$version, .DSVERT_CLIENT_DP_COX_GRID_CROSS_CONTRACT_VERSION) ||
...
.dsvert_dp_cox_grid_cross_formula <- function(formula) {
  ...
  reference <- function(value) {
    ...
    if (is.call(value) && length(value) == 3L &&
        identical(value[[1L]], quote(`$`)) && is.symbol(value[[2L]]) &&
        is.symbol(value[[3L]])) {
  ...
  if (!is.call(lhs) || length(lhs) != 3L ||
      !identical(lhs[[1L]], quote(Surv))) fail()
  terms <- stats::terms(formula)
  labels <- attr(terms, "term.labels")
  if (!length(labels) || any(!grepl(
      "^[A-Za-z.][A-Za-z0-9._]*(\\$[A-Za-z.][A-Za-z0-9._]*)?$", labels))) fail()
  # Terms must be literal additive columns, without transforms or specials.
  rhs <- function(value) {
    if (is.call(value) && identical(value[[1L]], quote(`+`)) &&
        length(value) == 3L) return(c(rhs(value[[2L]]), rhs(value[[3L]])))
    if (is.numeric(value) && length(value) == 1L && value %in% c(0, 1)) {
      return(character())
    }
    reference(value)
  }
  predictors <- rhs(formula[[3L]])
  if (!length(predictors) || anyDuplicated(predictors) ||
      !setequal(predictors, labels)) fail()
  list(time = reference(lhs[[2L]]), event = reference(lhs[[3L]]),
       predictors = predictors)
}

.dsvert_dp_cox_grid_cross_moment <- function(coordinates, spec) {
  ...
  if (!is.numeric(coordinates) || length(coordinates) != length(upper) ||
      !length(coordinates) || anyNA(coordinates) || any(!is.finite(coordinates)) ||
      any(coordinates != floor(coordinates)) || any(coordinates < 0) ||
      any(coordinates > upper)) .dsvert_dp_glm_grid_cross_fail()
  ...
  ranges <- vapply(spec$predictors, function(bound) bound$upper - bound$lower,
                   numeric(1L))
  if (length(beta) != length(ranges) || length(ranges) != length(predictors) ||
      any(!is.finite(ranges)) || any(ranges <= 0)) .dsvert_dp_glm_grid_cross_fail()
  coefficients <- stats::setNames(beta / ranges, predictors)
  list(coefficients = coefficients, hazard_ratio = exp(coefficients),
       normalized_coefficients = stats::setNames(beta, predictors),
       selected_candidate = as.integer(selected),
       selected_dp_partial_loss = coordinates[[selected]] /
         2^as.numeric(spec$numeric_grid_bits),
       candidate_selection = "first_canonical_noisy_minimum_v1")
}
...
.dsvert_dp_cox_grid_cross_impl <- function(
    formula, data_name, analysis_id, datasources,
    .release = .dsvert_dp_cox_grid_cross_release) {
  tryCatch({
    requested <- .dsvert_dp_cox_grid_cross_formula(formula)
    if (!.dsvert_dp_is_string(data_name) || !.dsvert_dp_is_string(analysis_id) ||
        !is.list(datasources) || !length(datasources) %in% 2:5 ||
        is.null(names(datasources)) || anyDuplicated(names(datasources))) {
      .dsvert_dp_glm_grid_cross_fail()
    }
    # This internal runner must return only an already authenticated existing
    # joint-DP release. Its argument is not exposed by the public API.
    released <- .release(data_name, analysis_id, datasources)
    contract <- .dsvert_dp_cox_grid_cross_contract_validate(
      released$contract, released$policy, released$schema_manifest)
    spec <- contract$spec
    matches <- function(name, descriptor) name %in% c(
    ...
    moment <- .dsvert_dp_cox_grid_cross_moment(released$coordinates, spec)
    result <- c(list(
      status = "public_certified_breslow_cox_finite_grid",
      analysis_id = analysis_id, family = "cox", ties = "breslow",
      time_semantics = spec$time_semantics), moment, list(
      signed_artifact = contract$artifact, signed_contract = contract,
...
dp_cox_grid <- function(formula, data, analysis_id, datasources = NULL) {
  tryCatch({
    ...
    .dsvert_dp_cox_grid_cross_impl(formula, resolved$value, analysis_id,
      .dsvert_dp_datasources(resolved$datasources))
  }, error = .dsvert_dp_glm_grid_cross_transcript_stop)
}
