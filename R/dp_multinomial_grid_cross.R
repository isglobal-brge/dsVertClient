...
.dsvert_dp_categorical_grid_cross_numeric <- function(family) {
  ...
  list(version = paste0("cross-grid-", family, "-piecewise-q16-v1"),
    base_profile_sha256 = .DSVERT_CLIENT_DP_GLM_GRID_CROSS_PROFILE_SHA256,
    base_certificate_sha256 = .DSVERT_CLIENT_DP_GLM_GRID_CROSS_CERTIFICATE_SHA256,
...
.dsvert_dp_categorical_grid_cross_candidates <- function(raw, family, p, k) {
  ...
  keys <- vapply(candidates, .dsvert_joint_dp_client_json, character(1L))
  ...
  encoded_keys <- vapply(encoded, .dsvert_joint_dp_client_json, character(1L))
...
.dsvert_dp_categorical_grid_cross_artifact <- function(spec) {
  list(version = paste0("bounded-", spec$family, "-cross-likelihood-grid-v1"),
    ...
    private_layout_sha256 = .dsvert_dp_capsule_source_hash(.dsvert_dp_glm_grid_cross_layout(spec)),
...
.dsvert_dp_categorical_grid_cross_contract_validate <- function(
    value, policy, schema_manifest, family, .verifier = .dsvert_dp_glm_grid_cross_verify) {
  tryCatch({
    .dsvert_dp_glm_grid_cross_fields(value, c("version", "spec", "artifact",
      "source_contract", "signatures"))
    if (!identical(value$version, .DSVERT_CLIENT_DP_GLM_GRID_CROSS_CONTRACT_VERSION)) {
      .dsvert_dp_glm_grid_cross_fail()
    }
    ...
    schema <- .dsvert_dp_glm_grid_cross_schema_validate(policy, schema_manifest$logical_snapshot,
...
.dsvert_dp_multinomial_grid_cross_register <- function() {
  list(family = "multinomial", spec_version = "multinomial_grid_cross_v1",
    ...
    postprocess = .dsvert_dp_multinomial_grid_cross_postprocess,
    release = .dsvert_dp_multinomial_grid_cross_release)
}

# Client integration boundary: public, authenticated descriptors only. The
# fused producer and typed two-authority release verifier belong to Step 2.
.dsvert_dp_categorical_grid_cross_formula <- function(formula, spec) {
  fail <- .dsvert_dp_glm_grid_cross_fail
  qualified <- function(term) {
    if (!is.call(term) || length(term) != 3L ||
        !identical(term[[1L]], as.name("$")) ||
        !is.symbol(term[[2L]]) || !is.symbol(term[[3L]])) fail()
    paste0(as.character(term[[2L]]), "$", as.character(term[[3L]]))
  }
  predictors <- function(term) {
    if (is.call(term) && identical(term[[1L]], as.name("+")) &&
        length(term) == 3L) {
      return(c(predictors(term[[2L]]), predictors(term[[3L]])))
    }
    qualified(term)
  }
  if (!inherits(formula, "formula") || length(formula) != 3L) fail()
  outcome <- qualified(formula[[2L]])
  variables <- predictors(formula[[3L]])
  if (anyDuplicated(variables) ||
      !identical(outcome, spec$outcome$reference) ||
      !setequal(variables, unlist(spec$predictor_order, use.names = FALSE))) fail()
  invisible(TRUE)
}

.dsvert_dp_categorical_grid_cross_postprocess <- function(contract, noisy_losses,
                                                        family) {
  tryCatch({
    spec <- contract$spec
    if (!identical(spec$family, family)) .dsvert_dp_glm_grid_cross_fail()
    .dsvert_dp_glm_grid_cross_equal(
      contract$artifact, .dsvert_dp_categorical_grid_cross_artifact(spec))
    upper <- unlist(spec$sensitivity$maximum_coordinates, use.names = FALSE)
    if (!is.numeric(noisy_losses) || !is.null(dim(noisy_losses)) ||
        !is.null(names(noisy_losses)) || length(noisy_losses) != length(upper) ||
        !length(noisy_losses) || anyNA(noisy_losses) ||
        any(!is.finite(noisy_losses)) || any(noisy_losses != floor(noisy_losses)) ||
        any(noisy_losses < 0 | noisy_losses > upper)) {
      .dsvert_dp_glm_grid_cross_fail()
    }
    candidate <- which.min(noisy_losses)[[1L]]
    terms <- unlist(spec$design_terms, use.names = FALSE)
    variables <- unlist(spec$predictor_order, use.names = FALSE)
    ranges <- vapply(spec$predictors, function(bound) {
      bound$upper - bound$lower
    }, numeric(1L))
    lowers <- vapply(spec$predictors, `[[`, numeric(1L), "lower")
    selected <- unlist(spec$beta_grid[[candidate]], use.names = FALSE)
    if (identical(family, "multinomial")) {
      levels <- unlist(spec$class_order, use.names = FALSE)
      non_reference <- setdiff(levels, spec$reference)
      normalized <- matrix(selected, nrow = length(terms),
        ncol = length(non_reference), dimnames = list(terms, non_reference))
      coefficients <- normalized
      coefficients[-1L, ] <- sweep(normalized[-1L, , drop = FALSE],
                                    1L, ranges, "/")
      coefficients[1L, ] <- normalized[1L, ] - colSums(sweep(
        coefficients[-1L, , drop = FALSE], 1L, lowers, "*"))
      parameters <- list(coefficients = coefficients,
        normalized_coefficients = normalized, reference = spec$reference,
        outcome_levels = levels)
    } else {
      normalized <- unlist(spec$candidate_grid[[candidate]]$beta,
                           use.names = FALSE)
      coefficients <- stats::setNames(normalized[-1L] / ranges, variables)
      normalized_thresholds <- unlist(
        spec$candidate_grid[[candidate]]$thresholds, use.names = FALSE)
      levels <- unlist(spec$class_order, use.names = FALSE)
      thresholds <- stats::setNames(normalized_thresholds - normalized[[1L]] +
        sum(coefficients * lowers), head(levels, -1L))
      parameters <- list(coefficients = coefficients,
        normalized_coefficients = normalized, thresholds = thresholds,
        normalized_thresholds = normalized_thresholds, ordered_levels = levels)
    }
    c(list(status = "ok", family = paste0(family, "_cross_owner_finite_grid"),
      analysis_id = spec$analysis_id, selected_candidate = as.integer(candidate),
      selected_dp_negative_log_likelihood = noisy_losses[[candidate]] /
        2^spec$numeric_grid_bits,
      candidate_selection = "minimum_signed_finite_grid_dp_postprocessing_v1",
      numeric_grid_bits = spec$numeric_grid_bits,
      coordinate_scale = 2^spec$numeric_grid_bits), parameters,
      list(covariance = NULL, std_errors = NULL, standard_errors = NULL,
        p_values = NULL, inference = "unavailable_for_finite_grid",
        source_values_exposed = FALSE, intermediate_values_exposed = FALSE,
        additional_privacy_cost = c(epsilon = 0, delta = 0),
        implementation_state = contract$artifact$implementation_state,
        cross_owner_state = contract$artifact$cross_owner_state,
        legacy_fallback_called = FALSE))
  }, error = .dsvert_dp_glm_grid_cross_transcript_stop)
}

.dsvert_dp_multinomial_grid_cross_postprocess <- function(contract, noisy_losses) {
...
#'   predictors are added across owners before the nonlinear loss. No optimizer
#'   accesses protected data. Candidate ties use signed canonical order.
#'   The signed nonlinearity uses 64 quadratic pieces and 32-bit words with
#'   16 fractional bits. Its uniform loss error is at most 0.0011 per row,
...
dp_multinomial_grid <- function(formula, data, analysis_id, signed_contract,
  ...
  tryCatch({
    contract <- .dsvert_dp_multinomial_grid_cross_contract_validate(
