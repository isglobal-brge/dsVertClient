...
.dsvert_capsule_method_inventory <- function() {
  ...
  add(
    ...
    c("signed_random_intercept_synopsis", "binary_outcome_bounds",
      "bounded_cluster_contributions", "signed_binary_random_slope_grid",
      "validated_synopsis_provenance"),
    paste(
      "Binary outcome ~ 1 population-average log-odds and observed-scale",
      "ICC, or additive fixed covariates selected from a signed finite",
      "Gauss-Hermite marginal-likelihood grid on the same-owner Synopsis."),
    c("binary_outcome_bounds", "fixed_random_intercept_or_one_or_two_signed_slope_scope",
      "finite_signed_covariate_grid", "identifiability",
      "sticky_dp_projection", "no_pql_or_unconstrained_likelihood",
      "no_more_than_two_random_slopes_or_sampling_inference"),
    "synopsis_release_implemented",
    character(),
  ...
  add(
    ...
    paste(
      ...
      "or a one-categorical-stratum saturated IPW/g-formula identity from",
      "one signed stratum-treatment-by-outcome contingency artifact."),
