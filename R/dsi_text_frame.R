...
.DSVERT_DSI_TEXT_REMOTE_FORMALS <- list(
  psiPaddedRelayExchangeDS = "request_json",
  exactGCCleanupDS = "cleanup_capability_json",
  dsvertDPCapsuleManifestSignDS =
    c("schema_json", "workload_contract_json"),
  dsvertDPCapsuleManifestBuildDS =
    c("schema_json", "workload_contract_json"),
  dsvertDPCapsuleSourceTicketDS = "manifest_json",
  dsvertDPCapsuleSourcePrepareDS = c(
    "manifest_json", "first_ticket_json", "second_ticket_json",
    "first_opening_json", "second_opening_json"),
  dsvertDPCapsuleSourceAcceptDS = "envelope_json",
  dsvertDPAlignmentMaskStartDS = "manifest_json",
  dsvertDPAlignmentMaskStoreDS = "manifest_json",
  dsvertDPAlignmentMaskSealDS = "manifest_json",
  dsvertDPAlignmentMaskReceiveDS = "manifest_json",
  dsvertDPCategoricalCrossBindDS =
    c("manifest_json", "first_opening_json", "second_opening_json"),
  dsvertDPCategoricalCrossFinalizeDS = "manifest_json",
  dsvertDPGaussianCrossBindDS = "manifest_json",
  dsvertDPGaussianCrossFinalizeDS = "manifest_json",
  dsvertJointDPVectorAllocationProofDS = "manifest_json",
  dsvertJointDPVectorAllocationPrepareDS =
    c("manifest_json", "leader_prepare_json"),
  dsvertJointDPVectorAllocationCommitDS =
    c("first_prepare_json", "second_prepare_json"),
  dsvertJointDPVectorAllocationAuthorizeDS =
    c("first_commit_json", "second_commit_json"),
  dsvertJointDPVectorAllocationOpenDS =
    c("first_authorization_json", "second_authorization_json"),
  dsvertJointDPVectorPrepareDS = c(
  ...
  dsvertJointDPVectorStartDS =
    c("manifest_json", "first_prepare_json", "second_prepare_json"),
  dsvertJointDPVectorResultDS =
    c("manifest_json", "first_prepare_json", "second_prepare_json"),
  dsvertJointDPVectorFinalShareDS =
    c("manifest_json", "first_result_json", "second_result_json"),
...
.dsvert_dsi_text_is_framed <- function(value) {
  is.character(value) && length(value) == 1L && !is.na(value) &&
    substr(value, 1L, 6L) %in% c(
      .DSVERT_DSI_TEXT_PREFIX_R, .DSVERT_DSI_TEXT_PREFIX_L,
      .DSVERT_DSI_TEXT_PREFIX_B)
}

.dsvert_dsi_text_frame_call <- function(expression) {
  method <- .dsvert_dsi_text_call_method(expression)
  required <- .DSVERT_DSI_TEXT_REMOTE_FORMALS[[method]]
  if (is.null(required)) return(expression)
  argument_names <- names(expression)
  if (is.null(argument_names) || anyDuplicated(argument_names[nzchar(
        argument_names)])) {
    stop("Invalid promoted DSI call arguments.", call. = FALSE)
  }
  for (argument in required) {
    index <- which(argument_names == argument)
    if (!length(index)) next
    if (length(index) != 1L ||
        .dsvert_dsi_text_is_framed(expression[[index]])) {
      stop("Invalid or already framed promoted DSI call.", call. = FALSE)
    }
    expression[[index]] <- .dsvert_dsi_text_encode(
      expression[[index]], paste(method, argument))
  }
  expression
}

.dsvert_dsi_text_frame_expressions <- function(expressions) {
  if (is.list(expressions) && !is.call(expressions)) {
    return(lapply(expressions, .dsvert_dsi_text_frame_call))
  }
  .dsvert_dsi_text_frame_call(expressions)
}

.dsvert_dsi_text_require_framed <- function(expressions) {
  values <- if (is.list(expressions) && !is.call(expressions)) {
    expressions
  } else {
    list(expressions)
  }
  for (expression in values) {
    ...
    for (argument in required) {
      ...
      if (length(index) == 1L &&
          !.dsvert_dsi_text_is_framed(expression[[index]])) {
        stop("Unframed promoted DSI call reached the transport boundary.",
             call. = FALSE)
      }
    }
  }
