...
.DSVERT_CLIENT_DP_NB_GRID_CROSS_PROFILE_SHA256 <- "36771f85ecc4edf240dc49a8fc2d49345f314ec5c8e758f4a918532902b3da17"
.DSVERT_CLIENT_DP_NB_GRID_CROSS_CERTIFICATE_SHA256 <- "1b21c0b8fe91b0f2f280c273469aaf0bad6a4deae176e3db2559ee83beea7dfd"
.DSVERT_CLIENT_DP_NB_GRID_CROSS_NUMERIC_SHA256 <- "369f10da36d7f4c12dd1989f3d838a90a8f1638ff80ace7cfbd4aef848d94ac1"
...
.dsvert_dp_nb_grid_cross_profile <- function() {
  path <- system.file("cross-grid-nb", "numeric_profile_nb_v1.json", package = "dsVertClient")
  if (!nzchar(path)) .dsvert_dp_glm_grid_cross_fail()
  value <- jsonlite::fromJSON(path, simplifyVector = FALSE)
  if (!identical(value$profile_sha256, .DSVERT_CLIENT_DP_NB_GRID_CROSS_PROFILE_SHA256) ||
      !identical(.dsvert_dp_capsule_source_hash(value$profile), .DSVERT_CLIENT_DP_NB_GRID_CROSS_PROFILE_SHA256) ||
      !identical(value$certificate_sha256, .DSVERT_CLIENT_DP_NB_GRID_CROSS_CERTIFICATE_SHA256) ||
      !identical(.dsvert_dp_capsule_source_hash(value$certificate), .DSVERT_CLIENT_DP_NB_GRID_CROSS_CERTIFICATE_SHA256) ||
      !identical(.dsvert_dp_capsule_source_hash(value$numeric_contract), .DSVERT_CLIENT_DP_NB_GRID_CROSS_NUMERIC_SHA256)) {
    .dsvert_dp_glm_grid_cross_fail()
  }
  value
}
...
.dsvert_dp_nb_grid_cross_spec <- function(raw, policy, schema) {
  ...
  result <- list(
    ...
    beta_encoded = lapply(beta_grid, function(beta) as.list(vapply(
      ...
      character(1L)))),
    candidate_order = as.list(vapply(candidates, .dsvert_dp_capsule_source_hash,
                                     character(1L))),
  ...
  encoded_keys <- vapply(Map(function(beta, theta) list(beta = beta, theta = theta),
    result$beta_encoded, result$theta_grid), .dsvert_joint_dp_client_json, character(1L))
...
.dsvert_dp_nb_grid_cross_artifact <- function(spec) {
  list(
    ...
    private_layout_sha256 = .dsvert_dp_capsule_source_hash(
    ...
    implementation_state = .DSVERT_CLIENT_DP_GLM_GRID_CROSS_IMPLEMENTATION_STATE,
    cross_owner_state = .DSVERT_CLIENT_DP_GLM_GRID_CROSS_STATE)
}
...
.dsvert_dp_nb_grid_cross_source_contract <- function(spec, artifact) {
  list(
    version = .DSVERT_CLIENT_DP_GLM_GRID_CROSS_SOURCE_VERSION,
    purpose = .DSVERT_CLIENT_DP_GLM_GRID_CROSS_SOURCE_PURPOSE,
...
.dsvert_dp_nb_grid_cross_contract_validate <- function(
    ...
    .verifier = .dsvert_dp_glm_grid_cross_verify) {
  tryCatch({
...
.dsvert_dp_nb_grid_cross_register <- function() {
  list(family = "nb", spec_version = "nb_grid_cross_v1",
       ...
       sensitivity = .dsvert_dp_nb_grid_cross_sensitivity,
       postprocess = .dsvert_dp_nb_grid_cross_moment,
       client_entry = dp_nb_grid,
       production_release_registered = FALSE)
}

# Postprocessing consumes a public, already authenticated and DP-released vector.
# The fused release adapter must validate its evidence before calling this helper.
.dsvert_dp_nb_grid_cross_moment <- function(coordinates, spec) {
  maximum <- unlist(spec$sensitivity$maximum_coordinates, use.names = FALSE)
  if (!is.numeric(coordinates) || length(coordinates) != length(spec$beta_grid) ||
      anyNA(coordinates) || any(!is.finite(coordinates)) ||
      any(coordinates != floor(coordinates)) || any(coordinates < 0) || any(coordinates > maximum)) {
    .dsvert_dp_glm_grid_cross_fail()
  }
  candidate <- which.min(coordinates)[[1L]]
  beta <- unlist(spec$beta_grid[[candidate]], use.names = FALSE)
  ranges <- vapply(spec$predictors, function(x) x$upper - x$lower, numeric(1L))
  lowers <- vapply(spec$predictors, `[[`, numeric(1L), "lower")
  slopes <- beta[-1L] / ranges
  terms <- unlist(spec$design_terms, use.names = FALSE)
  list(selected_candidate = as.integer(candidate),
       theta = spec$theta_grid[[candidate]],
       coefficients = stats::setNames(c(beta[[1L]] - sum(slopes * lowers), slopes), terms),
       normalized_coefficients = stats::setNames(beta, terms),
       selected_dp_negative_log_likelihood = coordinates[[candidate]] / 2^spec$numeric_grid_bits,
       candidate_selection = "minimum_signed_finite_grid_dp_postprocessing_v1",
       covariance = NULL, standard_errors = NULL, p_values = NULL,
       inference = "unavailable_for_finite_grid_nb2",
...
dp_nb_grid <- function(formula, data_name, analysis_id, signed_contract,
  ...
  tryCatch({
    contract <- .dsvert_dp_nb_grid_cross_contract_validate(
      signed_contract, policy, schema_manifest)
    if (!inherits(formula, "formula") || length(formula) != 3L ||
        !identical(data_name, contract$spec$dataset) ||
        !identical(analysis_id, contract$spec$analysis_id)) {
      .dsvert_dp_glm_grid_cross_fail()
    }
    outcome <- paste(deparse(formula[[2L]], width.cutoff = 500L), collapse = "")
    predictors <- attr(stats::terms(formula), "term.labels")
    if (!identical(outcome, contract$spec$outcome$reference) ||
        !setequal(predictors, unlist(contract$spec$predictor_order, use.names = FALSE)) ||
