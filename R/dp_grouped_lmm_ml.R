...
.dsvert_dp_grouped_grid_cross_ml_parameters <- function(value) {
  ...
  if (!identical(value$objective, "ml") || !is.list(value$variance_grid) ||
      ...
      length(value$variance_grid) > 256L) .dsvert_dp_grouped_grid_cross_fail()
  grid <- lapply(value$variance_grid, function(pair) {
    ...
    .dsvert_dp_grouped_grid_cross_parameters(pair, "lmm")
  })
...
.dsvert_dp_grouped_grid_cross_ml_numeric <- function(grouping, parameters) {
  out <- .dsvert_dp_grouped_grid_cross_numeric("lmm", grouping,
    parameters$variance_grid[[1L]])
  path <- system.file("certificates", "grouped_lmm_ml_v1.json", package = "dsVertClient")
...
.dsvert_dp_grouped_grid_cross_ml_sensitivity <- function(beta_grid, grid_bits,
  ...
  bounds <- unlist(lapply(parameters$variance_grid, function(pair) {
    fixed <- .dsvert_dp_grouped_grid_cross_sensitivity(beta_grid, "lmm", 1,
  ...
  if (any(!is.finite(c(caps, maxima, l1, l2))) ||
      any(c(caps, maxima, l1, l2) > 2^53-1)) .dsvert_dp_grouped_grid_cross_fail()
