...
.beaver_preprocessing_mode <- function(kind, n, p, ring) {
  ...
  mode <- match.arg(tolower(as.character(mode)[1L]), c("auto", "dealer", "ot"))
  if (identical(mode, "auto")) {
    # Direct Chou-Orlandi OT is intentionally bounded. It is useful as a
    # dealer-free correctness path, but full GLM/GEE validations need an OT
    # extension backend to make it production-fast.
    bit_ops <- as.numeric(n) * max(1, as.numeric(p)) * as.numeric(ring) * 2
    threshold <- getOption("dsvert.ot_direct_max_bit_ops", 512L)
    mode <- if (is.finite(bit_ops) && bit_ops <= threshold) "ot" else "dealer"
  }
  mode
}
