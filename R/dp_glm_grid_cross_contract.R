...
.DSVERT_CLIENT_DP_GLM_GRID_CROSS_SPEC_VERSIONS <- c(
...
.DSVERT_CLIENT_DP_GLM_GRID_CROSS_ARTIFACT_VERSIONS <- c(
...
.DSVERT_CLIENT_DP_GLM_GRID_CROSS_CONTRACT_VERSION <-
  "dsvert-cross-owner-grid-signed-contract-v1"
.DSVERT_CLIENT_DP_GLM_GRID_CROSS_SIGNING_DOMAIN <-
  "dsVert/cross-owner-grid-contract/v1|"
.DSVERT_CLIENT_DP_GLM_GRID_CROSS_SOURCE_VERSION <-
...
.DSVERT_CLIENT_DP_GLM_GRID_CROSS_LAYOUT_VERSION <-
  "dsvert-cross-grid-private-source-layout-v1"
.DSVERT_CLIENT_DP_GLM_GRID_CROSS_IMPLEMENTATION_STATE <-
  "cross_owner_exact_gc_materialized"
.DSVERT_CLIENT_DP_GLM_GRID_CROSS_STATE <- "exact_gc_to_joint_dp_vector_v1"
.DSVERT_CLIENT_DP_GLM_GRID_CROSS_PROFILE_SHA256 <-
...
.dsvert_dp_glm_grid_cross_equal <- function(value, expected) {
  if (!identical(.dsvert_joint_dp_client_json(
        .dsvert_joint_dp_client_canonical(value)),
      .dsvert_joint_dp_client_json(
        .dsvert_joint_dp_client_canonical(expected)))) {
    .dsvert_dp_glm_grid_cross_fail()
  }
  invisible(TRUE)
}
...
.dsvert_dp_glm_grid_cross_numeric <- function(family) {
  if (!family %in% names(.DSVERT_CLIENT_DP_GLM_GRID_CROSS_SPEC_VERSIONS) ||
  ...
  if (!identical(value$profile_sha256,
                 .DSVERT_CLIENT_DP_GLM_GRID_CROSS_PROFILE_SHA256) ||
      !identical(.dsvert_dp_capsule_source_hash(value$profile),
                 .DSVERT_CLIENT_DP_GLM_GRID_CROSS_PROFILE_SHA256) ||
...
.dsvert_dp_glm_grid_cross_spec <- function(
  ...
  family <- names(.DSVERT_CLIENT_DP_GLM_GRID_CROSS_SPEC_VERSIONS)[
    .DSVERT_CLIENT_DP_GLM_GRID_CROSS_SPEC_VERSIONS == raw$version]
  ...
  keys <- vapply(beta_grid, .dsvert_joint_dp_client_json, character(1L))
  ...
  result <- list(
    version = unname(.DSVERT_CLIENT_DP_GLM_GRID_CROSS_SPEC_VERSIONS[[family]]),
    ...
    beta_encoded = lapply(beta_grid, function(beta) as.list(vapply(
      ...
      character(1L)))),
    candidate_order = as.list(vapply(beta_grid, .dsvert_dp_capsule_source_hash,
                                     character(1L))),
...
.dsvert_dp_glm_grid_cross_layout <- function(spec) {
  ...
  list(
    version = .DSVERT_CLIENT_DP_GLM_GRID_CROSS_LAYOUT_VERSION,
...
.dsvert_dp_glm_grid_cross_artifact <- function(spec) {
  list(
    version = unname(.DSVERT_CLIENT_DP_GLM_GRID_CROSS_ARTIFACT_VERSIONS[[spec$family]]),
...
.dsvert_dp_glm_grid_cross_message <- function(unsigned) {
  charToRaw(paste0(.DSVERT_CLIENT_DP_GLM_GRID_CROSS_SIGNING_DOMAIN,
