# Read-only validation of server-minted per-analysis DP contracts.  The client
# never constructs the authoritative semantic contract or derives noise.
# Statistical calls cannot submit this internal contract directly.
...
.dsvert_dp_analysis_client_canonical_value_v1 <- function(value) {
  ...
  if (is.list(value)) {
    ...
    return(lapply(value, .dsvert_dp_analysis_client_canonical_value_v1))
  }
...
.dsvert_dp_analysis_client_scalar_id <- function(value) {
  is.character(value) && length(value) == 1L && !is.na(value) &&
    grepl("^[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}$", value)
}

.dsvert_dp_analysis_client_named_list <- function(
...
.dsvert_dp_analysis_client_reject_operational_fields <- function(value) {
  ...
  for (element in value) {
    .dsvert_dp_analysis_client_reject_operational_fields(element)
  }
  invisible(NULL)
}

.dsvert_dp_analysis_client_identity_pk <- function(value) {
  ...
  normalized <- if (grepl("^[A-Za-z0-9+/_-]{43}=?$", compact)) {
    candidate <- chartr("+/", "-_", sub("=$", "", compact))
    .dsvert_dp_normalize_identity_pk(candidate)
  } else {
    NULL
  }
  if (is.null(normalized)) {
    stop("Invalid identity public key in the analysis contract",
         call. = FALSE)
  }
  normalized
}

.dsvert_dp_analysis_client_positive_integer <- function(value) {
  if (!is.numeric(value) || length(value) != 1L || is.na(value) ||
      !is.finite(value) || value < 1 || value != floor(value) ||
      value > .Machine$integer.max) {
    stop("Invalid positive integer in the analysis contract",
         call. = FALSE)
  }
  as.numeric(value)
}
...
.dsvert_dp_analysis_frequency_levels_dimension_v1 <- function(
  ...
  for (level in levels) {
    ...
    if (length(level_bytes) != 1L || is.na(level_bytes) ||
        level_bytes > 16L * 1024L * 1024L - total_bytes) return(NULL)
...
.dsvert_dp_analysis_frequency_hash_v1 <- function(domain, value) {
  payload <- paste0(domain, .dsvert_joint_dp_client_json(value))
...
.dsvert_dp_analysis_frequency_profile_v1 <- function(primitive) {
  if (!.dsvert_dp_analysis_client_scalar_id(primitive)) return(NULL)
  common <- list(
...
.dsvert_dp_analysis_frequency_semantic_validate_v1 <- function(value) {
  ...
  canonical_levels <- if (utf8_exact) as.list(sort(
    unlist(raw_levels, use.names = FALSE), method = "radix")) else NULL
  if (!utf8_exact || anyDuplicated(unlist(raw_levels, use.names = FALSE)) ||
      !identical(raw_levels, canonical_levels)) fail()
  value <- tryCatch(.dsvert_dp_analysis_client_canonical_value_v1(value),
  ...
  if (!.dsvert_dp_analysis_frequency_object_v1(value, c(
      ...
      !.dsvert_dp_analysis_client_scalar_id(value$domain) ||
      !.dsvert_dp_analysis_client_scalar_id(value$cohort_id)) fail()
  .dsvert_dp_analysis_client_reject_operational_fields(value)
  ...
  if (is.null(profile) ||
      !all(vapply(arguments[c("dataset_id", "dataset_version", "variable_id")],
                  .dsvert_dp_analysis_client_scalar_id, logical(1L)))) fail()
  bounds <- arguments$coordinate_bounds
  bound <- tryCatch(.dsvert_dp_analysis_client_positive_integer(bounds$upper),
                    error = function(error) NULL)
  ...
  if (!.dsvert_dp_analysis_client_named_list(snapshots) ||
      length(snapshots) < 2L || length(snapshots) > 4096L) fail()
  owner_ids <- tryCatch(vapply(
    names(snapshots), .dsvert_dp_analysis_client_identity_pk, character(1L)),
  ...
  snapshot_valid <- all(vapply(snapshots, function(snapshot) {
    .dsvert_dp_analysis_frequency_object_v1(
      ...
      .dsvert_dp_analysis_client_scalar_id(snapshot$dataset_id) &&
      .dsvert_dp_analysis_client_scalar_id(snapshot$dataset_version) &&
      .dsvert_dp_analysis_frequency_hex_v1(snapshot$snapshot_commitment)
  }, logical(1L)))
  source <- tryCatch(.dsvert_dp_analysis_client_identity_pk(
  ...
  authority_ids <- tryCatch(vapply(
    roles$authority_ids, .dsvert_dp_analysis_client_identity_pk, character(1L)),
    error = function(error) character())
  ...
  if (!.dsvert_dp_analysis_frequency_object_v1(privacy, c(
      ...
      !.dsvert_dp_analysis_client_scalar_id(privacy$adjacency) ||
...
.dsvert_dp_analysis_client_gaussian_achieved_delta_v1 <- function(
...
.dsvert_dp_analysis_client_calibration_valid_v1 <- function(
  ...
  if (identical(mechanism$family, "gaussian")) {
    implementation_floor <-
      .dsvert_dp_analysis_client_gaussian_impl_delta_v1(
        coordinates, epsilon)
    achieved <- .dsvert_dp_analysis_client_gaussian_achieved_delta_v1(
      calibration$noise_scale, sensitivity, epsilon)
    tolerance <- 4096 * .Machine$double.eps *
      max(delta, .Machine$double.xmin)
    is.finite(implementation_floor) && is.finite(achieved) && delta > 0 &&
    ...
    identical(calibration$implementation_delta, 0) &&
...
.dsvert_dp_analysis_semantic_validate_v1 <- function(value) {
  ...
  value <- tryCatch(
    .dsvert_dp_analysis_client_canonical_value_v1(value),
    error = function(error) stop("Invalid canonical semantic contract",
                                 call. = FALSE))
  fields <- c(
    "version", "domain", "cohort_id", "owner_snapshots",
  ...
  valid <- is.list(value) && !is.null(names(value)) &&
    ...
    .dsvert_dp_analysis_client_scalar_id(value$domain) &&
    .dsvert_dp_analysis_client_scalar_id(value$cohort_id) &&
    .dsvert_dp_analysis_client_named_list(value$owner_snapshots) &&
    length(value$owner_snapshots) >= 2L &&
    length(value$owner_snapshots) <= 4096L
  if (!isTRUE(valid)) stop("Invalid semantic contract", call. = FALSE)
  .dsvert_dp_analysis_client_reject_operational_fields(value)

  snapshot_fields <- c(
    "version", "dataset_id", "dataset_version", "snapshot_commitment")
  owner_ids <- tryCatch(vapply(
    names(value$owner_snapshots), .dsvert_dp_analysis_client_identity_pk,
    character(1L)), error = function(error) character(0L))
  if (length(owner_ids) != length(value$owner_snapshots) ||
      anyDuplicated(owner_ids)) {
    stop("Invalid owner identities in the semantic contract",
         call. = FALSE)
  }
  names(value$owner_snapshots) <- owner_ids
  value$owner_snapshots <- value$owner_snapshots[
    order(names(value$owner_snapshots), method = "radix")]
  snapshots_valid <- all(vapply(
    names(value$owner_snapshots), function(owner) {
      snapshot <- value$owner_snapshots[[owner]]
      is.list(snapshot) && !is.null(names(snapshot)) &&
        setequal(names(snapshot), snapshot_fields) &&
        identical(snapshot$version, .DSVERT_DP_ANALYSIS_SNAPSHOT_VERSION) &&
        .dsvert_dp_analysis_client_scalar_id(snapshot$dataset_id) &&
        .dsvert_dp_analysis_client_scalar_id(snapshot$dataset_version) &&
        is.character(snapshot$snapshot_commitment) &&
        length(snapshot$snapshot_commitment) == 1L &&
        !is.na(snapshot$snapshot_commitment) &&
        grepl("^[0-9a-f]{64}$", snapshot$snapshot_commitment)
    }, logical(1L)))
  authorities <- tryCatch(vapply(
    if (is.list(value$noise_authorities) &&
        is.null(names(value$noise_authorities))) value$noise_authorities else
      list(),
    .dsvert_dp_analysis_client_identity_pk,
    character(1L)), error = function(error) character(0L))
  authorities <- unname(sort(authorities, method = "radix"))
  authorities_valid <- length(authorities) == 2L &&
    !anyDuplicated(authorities) && all(authorities %in% owner_ids) &&
    identical(
      authorities, unname(sort(owner_ids, method = "radix")[1:2]))
  value$noise_authorities <- as.list(authorities)
  analysis <- value$analysis
  analysis_valid <- is.list(analysis) && !is.null(names(analysis)) &&
    setequal(names(analysis), c(
      "primitive", "formula", "effective_arguments")) &&
    .dsvert_dp_analysis_client_scalar_id(analysis$primitive) &&
  ...
  privacy_valid <- is.list(privacy) && !is.null(names(privacy)) &&
    ...
    .dsvert_dp_analysis_client_scalar_id(privacy$adjacency) &&
    .dsvert_dp_analysis_client_scalar_id(privacy$privacy_unit) &&
    ...
    privacy$delta >= 0 && privacy$delta < 1
  contribution <- privacy$contribution
  contribution_valid <- is.list(contribution) &&
    !is.null(names(contribution)) && setequal(names(contribution), c(
    ...
    tryCatch({
      contribution$max_records_per_unit <-
        .dsvert_dp_analysis_client_positive_integer(
          contribution$max_records_per_unit)
      TRUE
    }, error = function(error) FALSE) &&
    .dsvert_dp_analysis_client_scalar_id(contribution$overflow_policy) &&
    contribution$overflow_policy %in%
      c("reject_operation", "clip_to_declared_bounds")
  constraints <- contribution$constraints
  constraints_valid <- is.list(constraints) && !is.null(names(constraints)) &&
    setequal(names(constraints), c("version", "policy_sha256")) &&
    identical(constraints$version, "dsvert-contribution-constraints-v1") &&
  ...
  mechanism <- privacy$mechanism
  mechanism_valid <- .dsvert_dp_analysis_client_named_list(mechanism) &&
    setequal(names(mechanism), c(
      "family", "version", "sensitivity", "calibration", "randomness")) &&
    .dsvert_dp_analysis_client_scalar_id(mechanism$family) &&
    mechanism$family %in% c("gaussian", "laplace", "discrete_laplace") &&
    .dsvert_dp_analysis_client_scalar_id(mechanism$version)
  sensitivity <- mechanism$sensitivity
  sensitivity_valid <- is.list(sensitivity) &&
    !is.null(names(sensitivity)) && setequal(
      names(sensitivity), c("version", "norm", "value")) &&
    identical(sensitivity$version, "dsvert-sensitivity-v1") &&
    is.character(sensitivity$norm) && length(sensitivity$norm) == 1L &&
    !is.na(sensitivity$norm) && sensitivity$norm %in% c("l1", "l2") &&
  ...
  calibration <- mechanism$calibration
  calibration_valid <- is.list(calibration) &&
    !is.null(names(calibration)) && setequal(names(calibration), c(
    ...
    .dsvert_dp_analysis_client_scalar_id(calibration$sampler) &&
  ...
  randomness_valid <- is.list(randomness) && !is.null(names(randomness)) &&
    setequal(names(randomness), c("version", "lanes")) &&
    identical(randomness$version, "dsvert-randomness-plan-v1") &&
    .dsvert_dp_analysis_client_named_list(randomness$lanes) &&
    "final_noise" %in% names(randomness$lanes)
  lanes_valid <- isTRUE(randomness_valid)
  if (isTRUE(lanes_valid)) {
    for (lane_name in names(randomness$lanes)) {
      lane <- randomness$lanes[[lane_name]]
      valid_lane <- .dsvert_dp_analysis_client_scalar_id(lane_name) &&
        is.list(lane) && !is.null(names(lane)) && setequal(
          names(lane), c(
      ...
      valid_lane <- isTRUE(valid_lane) &&
        identical(lane$purpose, expected_purpose) &&
        .dsvert_dp_analysis_client_scalar_id(lane$primitive) &&
        (!identical(lane_name, "final_noise") ||
         identical(lane$primitive, calibration$sampler))
      if (isTRUE(valid_lane)) {
        lane$coordinates <- tryCatch(
          .dsvert_dp_analysis_client_positive_integer(lane$coordinates),
          error = function(error) NULL)
        valid_lane <- !is.null(lane$coordinates)
      }
      if (!isTRUE(valid_lane)) {
        lanes_valid <- FALSE
        break
      }
      randomness$lanes[[lane_name]] <- lane
    }
  }
  mechanism$randomness <- randomness
  if (isTRUE(mechanism_valid && sensitivity_valid && calibration_valid &&
             lanes_valid)) {
    calibration_valid <-
      .dsvert_dp_analysis_client_calibration_valid_v1(
  ...
  numeric_valid <- is.list(numeric) && !is.null(names(numeric)) &&
    ...
    .dsvert_dp_analysis_client_scalar_id(numeric$sampler_encoding) &&
    .dsvert_dp_analysis_client_scalar_id(numeric$output_encoding)
  if (isTRUE(numeric_valid)) {
    numeric$value_bits <- tryCatch(
...
.dsvert_dp_analysis_artifact_key_v1 <- function(semantic) {
  ...
  digest::digest(
    object = charToRaw(paste0(
      .DSVERT_DP_ANALYSIS_ARTIFACT_DOMAIN,
      .dsvert_joint_dp_client_json(semantic))),
...
.dsvert_dp_analysis_execution_validate_v1 <- function(value, semantic) {
  value <- tryCatch(
    .dsvert_dp_analysis_client_canonical_value_v1(value),
    error = function(error) stop("Invalid canonical execution contract",
                                 call. = FALSE))
  valid <- is.list(value) && !is.null(names(value)) && setequal(
    names(value), c(
      "version", "peer_pins", "backend", "transport")) &&
    identical(value$version, .DSVERT_DP_ANALYSIS_EXECUTION_VERSION) &&
    .dsvert_dp_analysis_client_named_list(value$peer_pins) &&
    .dsvert_dp_analysis_client_named_list(value$backend) &&
    .dsvert_dp_analysis_client_named_list(value$transport)
  if (!isTRUE(valid)) stop("Invalid execution contract", call. = FALSE)
  normalized_pins <- tryCatch(vapply(
    value$peer_pins, .dsvert_dp_analysis_client_identity_pk,
    character(1L)), error = function(error) character(0L))
  ...
  transport$chunk_coordinates <- tryCatch(
    .dsvert_dp_analysis_client_positive_integer(
      transport$chunk_coordinates),
    error = function(error) NULL)
  if (is.null(transport$chunk_coordinates)) {
    stop("Invalid execution transport", call. = FALSE)
  }
  if (identical(
...
.dsvert_dp_analysis_contract_validate_v1 <- function(value) {
  value <- tryCatch(
    .dsvert_dp_analysis_client_canonical_value_v1(value),
    error = function(error) stop("Invalid canonical analysis contract",
                                 call. = FALSE))
  valid <- is.list(value) && !is.null(names(value)) && setequal(
    names(value), c("version", "artifact_key", "semantic", "execution")) &&
    identical(value$version, .DSVERT_DP_ANALYSIS_CONTRACT_VERSION) &&
    is.character(value$artifact_key) && length(value$artifact_key) == 1L &&
    !is.na(value$artifact_key) && grepl("^[0-9a-f]{64}$", value$artifact_key)
  if (!isTRUE(valid)) stop("Invalid analysis contract", call. = FALSE)
