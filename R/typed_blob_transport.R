...
.dsvert_typed_blob_resource_model <- function() {
  list(
    ...
    producer_capabilities = list(
      ...
      formal_finalizer_handoff_v1 = list(
        endpoints = c(
          "dsvertFormalFinalizerHandoffSourceDS", "mpcTypedBlobReadDS",
          "mpcTypedBlobStoreDS", "mpcTypedBlobReceiptDS"),
        producer_source_streaming = FALSE,
        client_source_streaming = TRUE,
        statistical_producer = TRUE,
        recipient_consumer_streaming = FALSE,
        producer_peak_memory = "O(envelope), bounded at 8 MiB raw",
        client_peak_memory = "O(frame)",
        maximum_outer_payload_chars = 11184811,
        payload_class = "authenticated recipient-AEAD ciphertext",
        statistical_frontdoors_connected = FALSE,
        production_ready = FALSE),
      formal_glm_one_draw_control_v1 = list(
        endpoints = c(
          "dsvertFormalGLMControlSourceDS", "mpcTypedBlobReadDS",
          "mpcTypedBlobStoreDS", "mpcTypedBlobReceiptDS"),
        producer_source_streaming = FALSE,
        client_source_streaming = TRUE,
        statistical_producer = TRUE,
    ...
    blocker = paste(
      ...
      "The data-free source probe proves the generic source layers. The formal",
      "finalizer handoff streams through the client but retains one bounded",
      "ciphertext in producer and consumer R memory; its statistical frontdoors",
      "remain disconnected. biomedical-capsule source v2 is the sole complete",
      "statistical chunk producer/consumer exception."))
}
...
# Internal formal GLM/Cox relay. The signed lifecycle has already selected the
# artifact and roles; this wrapper may choose only the pinned finalizer key and
# the ephemeral session. It reuses the one generic ticket/frame pump below.
.dsvert_relay_formal_finalizer_handoff_v1 <- function(
  ...
  if (!named_peer(producer_conn) || !named_peer(recipient_conn) ||
      identical(names(producer_conn), names(recipient_conn))) {
    stop("Formal-finalizer relay requires two distinct named pinned peers",
         call. = FALSE)
  }
  recipient_pk <- .dsvert_dp_normalize_identity_pk(recipient_pk)
  if (is.null(recipient_pk)) {
    stop("Formal-finalizer relay requires one canonical pinned recipient key",
         call. = FALSE)
  }
  if (!is.character(session_id) || length(session_id) != 1L ||
      is.na(session_id) ||
      !grepl("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$",
             session_id)) {
    stop("Formal-finalizer relay requires one canonical session identifier",
         call. = FALSE)
  }
  ...
  produced <- .dsvert_aggregate_strict(
    producer_conn,
    call(
      name = "dsvertFormalFinalizerHandoffSourceDS",
      recipient_pk = recipient_pk, session_id = session_id),
    operation = "formal-finalizer typed source ticket",
    .aggregate = .aggregate)
  value <- produced[[1L]]
  if (!is.list(value) || is.null(names(value)) ||
      anyDuplicated(names(value)) ||
      !identical(names(value), "source_transfer")) {
    stop("Formal-finalizer producer returned an invalid transfer envelope",
         call. = FALSE)
  }
  transfer <- .dsvert_validate_typed_blob_contract(
    value$source_transfer, names(recipient_conn))
  if (!identical(transfer$sender_name, names(producer_conn)) ||
      ...
      transfer$payload_chars > 11184811) {
    stop("Formal-finalizer producer returned a misbound transfer contract",
         call. = FALSE)
  }
  frames <- .dsvert_store_typed_blob_stream(
    transfer = transfer, conn = recipient_conn, session_id = session_id,
...
# Internal formal-GLM control relay. Rock has already selected the next
# lifecycle record and its direction; the client supplies only the fixed peer
# connection, its pinned transport key, and the ephemeral typed-blob session.
.dsvert_relay_formal_glm_control_v1 <- function(
  ...
  if (!named_peer(producer_conn) || !named_peer(recipient_conn) ||
      identical(names(producer_conn), names(recipient_conn))) {
    stop("Formal GLM control relay requires two distinct named pinned peers",
         call. = FALSE)
  }
  recipient_pk <- .dsvert_dp_normalize_identity_pk(recipient_pk)
  if (is.null(recipient_pk)) {
    stop("Formal GLM control relay requires one canonical pinned recipient key",
         call. = FALSE)
  }
  if (!is.character(session_id) || length(session_id) != 1L ||
      is.na(session_id) ||
      !grepl("^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$",
             session_id)) {
    stop("Formal GLM control relay requires one canonical session identifier",
         call. = FALSE)
  }
  ...
  produced <- .dsvert_aggregate_strict(
    producer_conn,
    call(
      name = "dsvertFormalGLMControlSourceDS",
      recipient_pk = recipient_pk, session_id = session_id),
    operation = "formal GLM control typed source ticket",
    .aggregate = .aggregate)
  value <- produced[[1L]]
  if (!is.list(value) || is.null(names(value)) ||
      anyDuplicated(names(value)) ||
      !identical(names(value), "source_transfer")) {
    stop("Formal GLM control producer returned an invalid transfer envelope",
         call. = FALSE)
  }
  transfer <- .dsvert_validate_typed_blob_contract(
    value$source_transfer, names(recipient_conn))
