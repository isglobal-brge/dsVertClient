...
.dsvert_dp_lasso_cross_base <- function(base, policy, family = NULL) {
  ...
  list(family = family, base_contract_sha256 = .dsvert_dp_capsule_source_hash(base),
...
.dsvert_dp_lasso_cross_spec <- function(raw, base) {
  ...
  beta_keys <- vapply(candidates, function(x) .dsvert_joint_dp_client_json(x$beta), character(1L))
  ...
  mapping <- if (identical(base$family, "gaussian")) NULL else
    as.list(match(beta_keys, vapply(base$beta_grid, .dsvert_joint_dp_client_json, character(1L))))
  ...
  list(version = "lasso_grid_cross_v1", analysis_id = raw$analysis_id,
    ...
    candidate_order = as.list(vapply(candidates, .dsvert_dp_capsule_source_hash, character(1L))),
