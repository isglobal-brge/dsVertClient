...
| **GLM** (gaussian / binomial / poisson) | `ds.vertGLM()` / `ds.vert.glm()` use a compatible same- or cross-owner Gaussian request with a custodian-configured `dp_analysis_id`, or read a completed binomial/Poisson formal certificate with `formal_analysis_id`. Neither route starts the retired iterative worker or provides sampling inference. |
