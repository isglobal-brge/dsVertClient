...
| **Survival** | Sticky fixed-grid DP survival artifacts and their Kaplan--Meier, Nelson--Aalen, competing-risk, survival-quantile, RMST and RMTL post-processors are available. `ds.vertCox()`, `ds.vert.cox()`, `ds.vert.coxph()` and `ds.vertCoxProfileNonDisclosive()` can read a completed dual-signed formal Cox certificate selected by `formal_analysis_id`. `ds.vertCoxDiscreteNonDisclosive()` and `ds.vert.cox(method = "discrete")` can instead read a separately bound two-authority binomial certificate for a fixed pooled-logistic time grid. Neither route starts a new run or provides classical inference, and the discrete route is not Cox PH. |
...
| GEE | Signed finite binomial/Poisson grid or completed formal certificate for independence, matching signed Gaussian random-intercept Synopsis for exchangeable working GLS, signed finite Gaussian AR(1) beta/rho working-GLS grid, or its distinct bounded bread/meat artifact | same read-only signed-artifact route | the AR(1) bread/meat route releases only componentwise-clipped cluster-score sandwich covariance when DP bread is positive definite; no standard errors, p-values or sampling inference, and no binomial/Poisson robust GEE |
