...
| **Multinomial** | `ds.vertMultinom()`, `ds.vertMultinomJoint()` and `ds.vertMultinomJointNewton()` accept `y ~ 1, frequency = ...` and post-process one validated sticky DP Frequency release into intercept-only log-odds. Covariate and joint-softmax routes remain unavailable pending a signed raw-design Gram workload. |
| **Ordinal (proportional odds)** | `ds.vertOrdinal()` and `ds.vertOrdinalJointNewton()` accept `y ~ 1, frequency = ..., levels_ordered = ...` and post-process one validated sticky DP Frequency release into cumulative-logit thresholds. Covariate proportional-odds routes remain unavailable pending a protected score/information artifact. |
| **Mixed / clustered models** | `ds.vertLMM()` / `ds.vert.lmm()` read signed same-owner random-intercept artifacts. `ds.vertGLMM()` / `ds.vert.glmm()` read signed binary random-intercept artifacts. `ds.vertGEE()` / `ds.vert.gee()` read a formal binomial/Poisson GLM as an independence-working point estimate, or a matching signed Gaussian random-intercept artifact as an exchangeable-working model-based GLS estimate. No route releases cluster statistics, sandwich covariance or sampling inference. |
...
| Multinomial | The ordinary and historical joint names support an intercept-only fit from one validated sticky Frequency release; slope route unavailable pending signed `multinomial_design_grams` | same | no covariates, covariance, standard errors or inference; unsafe local-moment/correlation reconstruction remains removed |
| Ordinal | The ordinary and historical Newton names support intercept-only cumulative-logit thresholds from one validated sticky Frequency release; covariate route unavailable pending a protected score/information artifact | same | caller supplies the complete clinical order; no covariance, standard errors or inference; warm final-estimator and patient-level joint reconstruction remain removed |
...
| GLMM | Signed binary `outcome ~ 1` population-average log-odds and observed-scale ICC moment approximation | same, tested through K=5 | not PQL/Laplace/ML/REML; no covariates, random slopes, covariance, standard errors or sampling inference |
