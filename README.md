...
| **Multinomial** | `ds.vertMultinom(y ~ 1, frequency = ...)` post-processes one validated sticky DP Frequency release into intercept-only log-odds. Covariate and joint-softmax routes remain quarantined pending a signed raw-design Gram workload. |
| **Ordinal (proportional odds)** | `ds.vertOrdinal(y ~ 1, frequency = ..., levels_ordered = ...)` post-processes one validated sticky DP Frequency release into cumulative-logit thresholds. Covariate proportional-odds routes remain quarantined pending a protected score/information artifact. |
...
| Multinomial | Intercept-only fit from one validated sticky Frequency release; slope route unavailable pending signed `multinomial_design_grams` | same | no covariates, covariance, standard errors or inference; unsafe local-moment/correlation reconstruction remains removed |
