...
| **Causal / robustness** | `ds.vertIPW(outcome ~ treatment, treatment ~ 1, ...)` is available only for the exact binary intercept-only ATE identity over one signed sticky treatment-by-outcome DP table; it releases no weights or propensity fit. `ds.vertMI()` / `ds.vert.mi()` provide categorical MCAR independent-marginal completions for `outcome ~ 1` or `cbind(...) ~ 1` from one signed Synopsis release. Covariate IPW, ATT/ATC, joint/chained MI and Rubin inference remain unavailable. |
