...
| GLMM | Signed binary `outcome ~ 1` population-average log-odds/ICC moment approximation, additive binary bare-covariate finite random-intercept/one-to-three-random-slope grid, or additive bounded-count Poisson random-intercept grid | same, tested through K=5 | not PQL/Laplace/ML/REML; no interactions, Poisson random slopes, more than three binary random slopes, covariance, standard errors or sampling inference |
