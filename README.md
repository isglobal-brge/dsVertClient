...
| **Inference helpers** | `ds.vertConfint(type = "mechanism")`, `ds.vertWald(type = "mechanism")` and `ds.vertContrast(type = "mechanism")` revalidate the signed same-owner Gaussian Synopsis and return simultaneous DP-mechanism regions with no extra release. They are not sampling inference, Wald statistics or p-values. `ds.vertLR()` remains provisional until a jointly attested nested-model deviance artifact exists. |
