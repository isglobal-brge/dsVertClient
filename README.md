...
choosing a route: its method-specific release contract and limitation are
authoritative. A `promoted` route is available only within its declared
numerical, disclosure and biomedical scope; historical routes outside that
scope remain unavailable rather than silently falling back.
...
# ds.vertDPLogRank(surv_treated, surv_control, cause = "event")
```
...
| **Differential privacy** | `ds.vertDPStatus()`, `ds.vertDPCapsulePlan()`, `ds.vertDPCalibrate()`, `ds.vertDPCount()`, `ds.vertDPContingency()`, `ds.vertDPFrequency()`, `ds.vertDPFrequencyInference()`, `ds.vertDPMeanVar()`, `ds.vertDPDescribe()`, `ds.vertDPQuantile()`, `ds.vertDPMedian()`, `ds.vertDPGaussian()`, `ds.vertDPSurvival()`, `ds.vertDPSurvivalQuantile()`, `ds.vertDPMedianSurvival()`, `ds.vertDPSurvivalContrast()`, `ds.vertDPRMSTContrast()`, `ds.vertDPLogRank()`, `ds.vertDPEpi2x2()`, `ds.vertDPEpi2x2Inference()`, `ds.vertDPMantelHaenszel()`, `ds.vertDPDiagnostic2x2()`, `ds.vertDPDiagnostic2x2Inference()`, `ds.vertDPROC()`, `ds.vertDPDirectStandardizationInference()`, `ds.vertDPIndirectStandardizationInference()`, `ds.vertDPCausalStandardization()`, `ds.vertDPCausalStandardizationInference()` and the remaining direct/indirect standardisation, competing-risk, RMST, and RMTL post-processors |
| **GLM** (gaussian / binomial / poisson) | `ds.vertGLM()` / `ds.vert.glm()` use a compatible same- or cross-owner Gaussian request with a custodian-configured `dp_analysis_id`, select binomial/Poisson coefficients from a signed finite likelihood grid with `analysis_id`, or read a completed binomial/Poisson formal certificate with `formal_analysis_id`. `fresh_formal_analysis_id` is sealed before DSI until its protected runtime is production-attested. No available route exposes analyst-owned privacy/MPC controls, starts the retired iterative worker, or provides sampling inference. |
| **Inference helpers** | `ds.vertConfint(type = "mechanism")`, `ds.vertWald(type = "mechanism")`, `ds.vertContrast(type = "mechanism")` and `ds.vertLR(type = "mechanism")` revalidate the signed same-owner Gaussian Synopsis and return simultaneous DP-mechanism regions with no extra release. The LR route is an outer region for a bounded normalized RSS reduction from one release, never a likelihood-ratio statistic, chi-square reference or p-value. |
...
| **Survival** | Sticky fixed-grid DP survival artifacts and their Kaplan--Meier, Nelson--Aalen, competing-risk, survival-quantile, RMST and RMTL post-processors are available. `ds.vertCox()`, `ds.vert.cox()`, `ds.vert.coxph()` and `ds.vertCoxProfileNonDisclosive()` select a same-owner signed Breslow partial-likelihood finite grid with `analysis_id`, or read a completed dual-signed formal Cox certificate with `formal_analysis_id`. `ds.vertCoxDiscreteNonDisclosive()` and `ds.vert.cox(method = "discrete")` can instead read a separately bound two-authority binomial certificate for a fixed pooled-logistic time grid. Neither route starts a new run or provides classical inference, and the discrete route is not Cox PH. |
| **Negative binomial** | `ds.vertNBFullRegTheta(y ~ 1, frequency = ...)` and `ds.vert.nb(y ~ 1, frequency = ...)` post-process one validated sticky DP Frequency release into an intercept-only NB2 method-of-moments fit. Additive covariates are also available only through a custodian-signed finite `analysis_id` beta/theta likelihood grid using [corrected v2 loss and sensitivity semantics](inst/docs/negative_binomial_grid_v2.md); v1 is sealed/defective and rejected. There is no iterative remote likelihood or sampling inference. |
| **Multinomial** | `ds.vertMultinom()`, `ds.vertMultinomJoint()` and `ds.vertMultinomJointNewton()` post-process `y ~ 1, frequency = ...` into intercept-only log-odds. With `analysis_id`, the same names select additive bare-predictor softmax coefficients from one same-owner signed finite likelihood grid. There is no unconstrained optimiser, covariance, standard errors or sampling inference. |
| **Ordinal (proportional odds)** | `ds.vertOrdinal()` and `ds.vertOrdinalJointNewton()` post-process `y ~ 1, frequency = ..., levels_ordered = ...` into cumulative-logit thresholds. With `analysis_id`, they select additive bare-predictor cumulative-logit coefficients and ordered thresholds from one same-owner signed finite likelihood grid. There is no unconstrained optimiser, covariance, standard errors or sampling inference. |
| **Mixed / clustered models** | `ds.vertLMM()` / `ds.vert.lmm()` read signed same-owner random-intercept, fixed-effect or finite random-slope artifacts. `ds.vertGLMM()` / `ds.vert.glmm()` read a signed binary `outcome ~ 1` moment artifact, an additive binary finite random-intercept/one-to-three-random-slope likelihood grid, or an additive bounded-count Poisson random-intercept/one-to-three-random-slope grid. `ds.vertGEE()` / `ds.vert.gee()` read a formal or signed-grid binomial/Poisson GLM as an independence-working point estimate, a matching signed Gaussian random-intercept artifact as an exchangeable-working model-based GLS estimate, a signed Gaussian AR(1) grid, or a matching same-owner binomial/Poisson robust-independence grid with one categorical cluster id. Robust grids provide componentwise-clipped cluster-score sandwich covariance only when DP bread is positive definite. They never provide standard errors, p-values or sampling inference. |
| **Causal / robustness** | `ds.vertIPW(outcome ~ treatment, treatment ~ 1, ...)` provides the exact binary intercept-only ATE identity; `treatment ~ stratum` additionally provides exact one-categorical-stratum saturated IPW/g-formula ATE, ATT and ATC identities from one signed stratum–treatment–outcome table. ATE uses fixed public target weights with a combined DP-plus-sampling region; ATT/ATC derive target-arm weights from the sticky DP table and provide a DP-mechanism region only. Neither releases weights or a propensity fit. `ds.vertMI()` / `ds.vert.mi()` provide categorical MCAR marginal completions, a two-response signed joint pair, or an opt-in conditional categorical star model from strict-missing pairs in one signed Synopsis release. Continuous/multiple-covariate IPW, chained MI and Rubin inference remain unavailable. |
| **Penalised regression** | `ds.vertLASSOIter()` admits a promoted Gaussian Synopsis path when `dp_analysis_id` selects the same-owner artifact, and a promoted binomial/Poisson finite L1 candidate path when `analysis_id` selects its signed artifact; `lambda` may only select an exact signed penalty level. `ds.vertLASSO()` and `ds.vertLASSO1Step()` remain Gaussian post-processors. The same-owner Gaussian Synopsis post-processors `ds.vertLASSOProximal()` and the information-criterion (not cross-validation) selector `ds.vertLASSOCV()` are promoted without sampling inference. |

## K=2 vs K≥3 support

| Family | K=2 product route | K≥3 product route | Legacy / not offered |
|---|---|---|---|
| GLM (gauss / binom / poisson) | Compatible Gaussian signed-Synopsis adapter, signed finite binomial/Poisson grid, or read-only completed certificate | same, tested through K=5 | fresh formal and legacy iterative routes are sealed |
| Cox PH | Same-owner signed Breslow finite grid or read-only completed dual-signed certificate route | same, tested through K=5 | no arbitrary optimiser, covariance, p-values, baseline hazard or `legacy_rank` route |
| Discrete hazard | Read-only completed two-authority binomial certificate bound to a fixed grid | same | pooled-logistic estimand only; no person-period expansion, Cox-PH claim or sampling inference |
| Negative binomial | Intercept-only NB2 method-of-moments fit from one validated sticky count Frequency release, or additive coefficients/theta selected from one bounded signed finite grid | same | no unconstrained likelihood optimization, covariance, standard errors or sampling inference; disclosive `variant = "full_reg"` remains removed |
| Multinomial | Intercept-only fit from one validated sticky Frequency release, or additive bare-predictor softmax coefficients from one same-owner signed finite likelihood grid | same, tested through K=5 | no interactions/transforms, unconstrained optimisation, covariance, standard errors or sampling inference; unsafe local-moment/correlation reconstruction remains removed |
| Ordinal | Intercept-only cumulative-logit thresholds from one validated sticky Frequency release, or additive bare-predictor cumulative-logit coefficients/thresholds from one same-owner signed finite likelihood grid | same, tested through K=5 | caller supplies the complete clinical order; no interactions/transforms, unconstrained optimisation, covariance, standard errors or sampling inference |
| LMM | Signed same-owner random-intercept `outcome ~ 1` moment artifact, fixed-effect finite ML/REML variance-ratio grid, or finite Gaussian random-slope candidate grid | same signed-artifact route, tested through K=5 | no unrestricted optimisation/covariance, standard errors or sampling inference |
| GEE | Signed finite binomial/Poisson grid or completed formal certificate for independence, matching signed Gaussian random-intercept Synopsis for exchangeable working GLS, signed finite Gaussian AR(1) beta/rho working-GLS grid, or a same-owner binomial/Poisson robust-independence bread/meat grid | same read-only signed-artifact route, tested through K=5 | Robust grids release componentwise-clipped cluster-score sandwich covariance only when DP bread is positive definite; no standard errors, p-values or sampling inference |
| GLMM | Signed binary `outcome ~ 1` population-average log-odds/ICC moment approximation, additive binary or bounded-count Poisson bare-covariate finite random-intercept/one-to-three-random-slope grid | same, tested through K=5 | not PQL/Laplace/ML/REML; no interactions, more than three random slopes, standard errors or sampling inference |
...
the promoted Count, PSI and Synopsis routes,
