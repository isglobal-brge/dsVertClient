...
| **Causal / robustness** | `ds.vertIPW(outcome ~ treatment, treatment ~ 1, ...)` is available only for the exact binary intercept-only ATE identity over one signed sticky treatment-by-outcome DP table; it releases no weights or propensity fit. `ds.vertMI()` / `ds.vert.mi()` provide categorical MCAR marginal completions, a two-response signed joint pair, or an opt-in conditional categorical star model from strict-missing pairs in one signed Synopsis release. Covariate IPW, ATT/ATC, chained MI and Rubin inference remain unavailable. |
