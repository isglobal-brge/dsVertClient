...
| **Mixed / clustered models** | `ds.vertLMM()` / `ds.vert.lmm()` read a signed same-owner `outcome ~ 1` random-intercept moment artifact. `ds.vertGLMM()` / `ds.vert.glmm()` read that artifact only for a binary population-average log-odds and observed-scale ICC approximation. `ds.vertGEE()` / `ds.vert.gee()` read a completed or matching configured fresh formal binomial/Poisson GLM analysis as an independence-working GEE point estimate. Clustered GEE and mixed-model likelihood/inference remain unavailable. |
...
| LMM | Signed same-owner `outcome ~ 1` random-intercept moment artifact | same, tested through K=5 | no ML/REML, random slopes, covariance, standard errors or sampling inference |
