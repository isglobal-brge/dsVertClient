...
| **Negative binomial** | `ds.vertNBFullRegTheta(y ~ 1, frequency = ...)` and `ds.vert.nb(y ~ 1, frequency = ...)` post-process one validated sticky DP Frequency release into an intercept-only NB2 method-of-moments fit. Covariate NB2 regression and inference remain quarantined pending a protected score/information artifact. |
...
| Negative binomial | Intercept-only NB2 method-of-moments fit from one validated sticky count Frequency release; covariate route unavailable pending a bounded NB2 capsule | same | no likelihood optimization, covariance, standard errors or inference; disclosive `variant = "full_reg"` remains removed |
