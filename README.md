...
| **GLM** (gaussian / binomial / poisson) | `ds.vertGLM()` / `ds.vert.glm()` use a compatible same- or cross-owner Gaussian request with a custodian-configured `dp_analysis_id`, read a completed binomial/Poisson formal certificate with `formal_analysis_id`, or resume one custodian-configured durable binomial/Poisson analysis with `fresh_formal_analysis_id`. No route exposes analyst-owned privacy/MPC controls, starts the retired iterative worker, or provides sampling inference. |
...
|---|---|---|---|
| GLM (gauss / binom / poisson) | Compatible Gaussian signed-Synopsis adapter or read-only completed binomial/Poisson certificate | same, tested through K=5 | fresh legacy iterative routes are not production claims |
...
| GEE | Formal binomial/Poisson certificate for independence, or matching signed Gaussian random-intercept Synopsis for exchangeable working GLS | same read-only signed-artifact route | no cluster statistics, robust covariance, standard errors, AR(1) correlation or inference |
