...
| **Causal / robustness** | `ds.vertIPW(outcome ~ treatment, treatment ~ 1, ...)` provides the exact binary intercept-only ATE identity; `treatment ~ stratum` additionally provides its exact one-categorical-stratum saturated IPW/g-formula identity from a signed stratum–treatment–outcome table and fixed public target weights. Neither releases weights or a propensity fit. `ds.vertMI()` / `ds.vert.mi()` provide categorical MCAR marginal completions, a two-response signed joint pair, or an opt-in conditional categorical star model from strict-missing pairs in one signed Synopsis release. ATT/ATC, continuous/multiple-covariate IPW, chained MI and Rubin inference remain unavailable. |
