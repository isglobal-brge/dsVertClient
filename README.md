...
| **Inference helpers** | `ds.vertConfint(type = "mechanism")` returns a certificate-revalidated simultaneous DP-mechanism region for the signed same-owner Gaussian Synopsis. It is not sampling inference. `ds.vertContrast()`, `ds.vertWald()` and `ds.vertLR()` remain provisional until a joint attested covariance/deviance artifact exists. |
