...
| **Negative binomial** | `ds.vertNBFullRegTheta(y ~ 1, frequency = ...)` and `ds.vert.nb(y ~ 1, frequency = ...)` post-process one validated sticky DP Frequency release into an intercept-only NB2 method-of-moments fit. Additive covariates are also available only through a custodian-signed finite `analysis_id` beta/theta likelihood grid; there is no iterative remote likelihood or sampling inference. |
