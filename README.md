...
| **Causal / robustness** | `ds.vertIPW(outcome ~ treatment, treatment ~ 1, ...)` is available only for the exact binary intercept-only ATE identity over one signed sticky treatment-by-outcome DP table; it releases no weights or propensity fit. Covariate IPW, ATT/ATC and MI remain unavailable. |
