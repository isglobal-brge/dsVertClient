...
| GEE | Signed finite binomial/Poisson grid or completed formal certificate for independence, matching signed Gaussian random-intercept Synopsis for exchangeable working GLS, or signed finite Gaussian AR(1) beta/rho working-GLS grid | same read-only signed-artifact route | no cluster statistics, robust covariance, standard errors or inference; AR(1) is finite-grid working GLS, not robust GEE |
