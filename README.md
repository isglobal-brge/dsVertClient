...
| GLMM | Signed binary `outcome ~ 1` population-average log-odds/ICC moment approximation, additive binary bare-covariate finite random-intercept/one-to-three-random-slope grid, or additive bounded-count Poisson random-intercept/one-random-slope grid | same, tested through K=5 | not PQL/Laplace/ML/REML; no interactions, more than one Poisson or three binary random slopes, standard errors or sampling inference |
